...
def build():
    ...
    s.text(Wd-24,28,"Riverside Gardens Apartments · RCS 5th-year renewal",10,"#9fb4d0",anchor="end")
    by=56; bh=212
    ...
    chk=[("Property name","identical on both uploads & record",True),("FHA / Section 8 #","matches the stored record",True),("Signatory (Part H)","Alex Morgan on prior & new rent schedule",True),("Utility allowance","exec schedule $33 vs RCS $31 — using exec",False),("SAFMR (150% ceiling)","HUD $2,330 vs RCS $2,290 — using HUD",False)]
    ...
    colbody(s,X,b,CW,[("Property name","Riverside Gardens Apartments","db"),("Address","100 Main Street, Wilmette IL 60091","db"),("FHA / Section 8 #","IL00X000000","db")],[("Ownership entity","Riverside Gardens Preservation, L.P.","db"),("Entity type","Limited Partnership","db"),("Entity address","30 Hudson Yards, NY","db")]); y+=h+G
    # 3 poc
    b,h=seccard(s,X,y,CW,156,3,"Point of contact & signatory",'ok')
    colbody(s,X,b,CW,[("Point of contact","Jordan Doe","db"),("Email","jdoe@example.com","db"),("Phone","(929) 618-8405","db")],[("General Partner","Related (GP)","db"),("Signatory","Alex Morgan","db"),("Signatory title","Vice President","db")]); y+=h+G
    # 4 contract admin
    b,h=seccard(s,X,y,CW,156,4,"Contract administrator",'attn')
    lx=X+22; rx=X+CW*0.5+10; colw=CW*0.5-40
    fbox(s,lx,b+22,colw,"CA organization","Regional Housing Agency","db")
    ...
    s.line(lx+70,b+72,lx+70,b+93,bd); s.text(lx+80,b+87,"Sam Rivera",11,INK)
    ...
    colbody(s,X,b,CW,[("Appraisal company","Belfry Valuation","cyc"),("Appraiser name","Taylor Reed","cyc")],[("Email","appraiser@example.com","cyc"),("Phone","(708) 500-2380","cyc")]); y+=h+G
    # 6 rents
    ...
    tw=(CW-44)/3
    fbox(s,X+22,b+24,tw-16,"Sender — name","Jordan Lee","db"); fbox(s,X+22+tw,b+24,tw-16,"Sender — title","Community Manager","db"); fbox(s,X+22+2*tw,b+24,tw-16,"Management address","100 Main Street","db"); y+=h+G
