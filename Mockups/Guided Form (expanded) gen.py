...
def build():
    ...
    s.text(24,80,"Reviewing — Riverside Gardens Apartments",13.5,INK,weight="700"); s.text(24,96,"every field shown expanded for review",9.5,SUB)
    gx=Wd-24
    ...
    fld(s,X+12,y+14,hw,"Property name","Riverside Gardens Apartments","db"); fld(s,X+28+hw,y+14,hw,"FHA / Section 8 #","IL00X000000","db")
    fld(s,X+12,y+58,hw,"Address","100 Main Street, Wilmette IL 60091","db"); fld(s,X+28+hw,y+58,hw,"Entity type","Limited Partnership","db")
    fld(s,X+12,y+102,hw,"Ownership entity","Riverside Gardens Preservation, L.P.","db"); fld(s,X+28+hw,y+102,hw,"Entity address","30 Hudson Yards, 72nd Fl, NY","db"); y+=150

    # 3 Owner & signatory
    y+=exhead(s,X,y,CW,3,"Owner & signatory",'ok')
    fld(s,X+12,y+14,hw,"Owner point of contact","Jordan Doe","db"); fld(s,X+28+hw,y+14,hw,"POC phone · auto-format","(929) 618-8405","db")
    fld(s,X+12,y+58,hw,"POC email","jdoe@example.com","db"); fld(s,X+28+hw,y+58,hw,"General Partner","Related (GP)","db")
    fld(s,X+12,y+102,hw,"Signatory — name","Alex Morgan","db"); fld(s,X+28+hw,y+102,hw,"Signatory — title","Vice President","db"); y+=150

    # 4 Contract admin
    y+=exhead(s,X,y,CW,4,"Contract administrator",'attn')
    fld(s,X+12,y+14,hw,"Contact","Ms. Sam Rivera","db"); fld(s,X+28+hw,y+14,hw,"Position","Asset Manager","db")
    fld(s,X+12,y+58,hw,"CA organization","Regional Housing Agency","db"); fld(s,X+28+hw,y+58,hw,"Salutation (auto)","Dear Ms. Gross","calc")
    ...
    fld(s,X+12,y+14,hw,"Appraiser name","Taylor Reed","cyc"); fld(s,X+28+hw,y+14,hw,"Firm","Belfry Valuation","cyc")
    fld(s,X+12,y+58,hw,"Email","appraiser@example.com","cyc"); fld(s,X+28+hw,y+58,hw,"Phone · auto-format","(708) 500-2380","cyc"); y+=106
    ...
    fld(s,X+12,y+58,hw,"Sender — name","Jordan Lee","db"); fld(s,X+28+hw,y+58,hw,"Sender — position","Community Manager","db")
    fld(s,X+12,y+102,hw,"Comments to (mgmt) · derived","RA Management, 100 Main Street","calc"); fld(s,X+28+hw,y+102,hw,"Copy to CA · derived","NHC, Tucker GA","calc"); y+=156
