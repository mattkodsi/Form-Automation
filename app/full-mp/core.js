...
function makeStore(adapter, FIELDS) {
  ...
  return {
    ...
    async saveField(form,key){ const db=await adapter.getDb(); const v=(form[key]||blank()).value; db[key]={value:(v==null?'':v),source:'database',saved_at:today()}; await adapter.saveDb(db); form[key]={value:db[key].value,source:db[key].value===''?'new':'database',saved_at:db[key].saved_at,prior_value:null,prior_source:null,db_value:db[key].value}; return form; },
    ...
    async saveFields(form,keys){ const db=await adapter.getDb();
      ...
      for(const key of keys){ const v=(form[key]||blank()).value; db[key]={value:(v==null?'':v),source:'database',saved_at:today()}; }
    ...
    async saveToDb(form){ const db=await adapter.getDb();
      for(const key of Object.keys(form)){ const v=(form[key]||blank()).value; db[key]={value:(v==null?'':v),source:'database',saved_at:today()}; }
      await adapter.saveDb(db);
