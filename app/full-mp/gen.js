...
const addrLine=(street,city,state,zip)=>{
  ...
  async function fillRentSchedule(templateBytes, rec){
    ...
    // Flattened: RA lock wins, then the one value (date_eff_custom), then legacy fallbacks for old records.
  ...
  const effOf=rec=>{const g=k=>rec[k]!=null?String(rec[k]):'';const de=(g('rent_schedule.date_eff_ra')||g('rent_schedule.date_eff_custom')||g('rent_schedule.date_eff_rs')||g('rent_schedule.date_rents_effective'));   // flattened: RA lock, then the one value, then legacy fallbacks
