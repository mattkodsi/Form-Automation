...
function coupledKeys(k){   // UA and SAFMR flattened: units.N.ua_custom / safmr_custom are plain cells, coupled to nothing
...
function srcSpec(k){
  /* UA and SAFMR are no longer resolver cells (flattened 2026-08-07): units.N.ua_custom and
     units.N.safmr_custom are plain money cells, so neither returns a spec here — typing stays a plain
     edit and their dropdown picks fall through to the table-driven engine (moneySrcRows). */
  let m;
  if(k==='rent_schedule.date_eff_custom')return{srcKey:'rent_schedule.date_eff_source',cusKey:k,
    fallback:'rs',names:['rs'],resolve:()=>fmtDate(get('rent_schedule.date_eff_rs')),figKey:()=>'rent_schedule.date_eff_rs'};
...
function wireBody(){
  ...
  document.querySelectorAll('[data-srcopt]').forEach(o=>o.addEventListener('click',e=>{e.stopPropagation();
    ...
    _pending=[key];_refocusSel='[data-box="'+key+'"] .uatrigger';renderBody();setStatus('Pulled from '+((o.querySelector('.uasub')||{}).textContent||'the source')+'.');}));
