...
function sourcesFor(key){
  ...
  if(brows.length){
    const cur=form[key];const org=cur&&cur.origin;
    const rows=brows.map(r=>{const oc=TAG_ORIGIN[r.tag];
      return {kind:'doc',src:oc||r.tag,tag:r.tag,val:(r.val==null||r.val==='')?null:r.val,sel:!!oc&&org===oc};});
    rows.push({kind:'free',src:'custom',tag:'Custom',val:cur?cur.value:'',sel:org==='typed'});
    return rows;
  }
...
function sourceMenu(key){
  const lines=sourcesFor(key); if(!lines.length)return '';
  ...
  const opts=lines.map(l=>{
    ...
    return l.val!=null
      ?'<div class="uaopt srcopt'+(l.sel?' sel':'')+'" '+attrs+'>'+esc(srcDisp(key,l.val))+'<span class="uasub">'+esc(l.tag)+'</span>'+lock(l.sel)+'</div>'
