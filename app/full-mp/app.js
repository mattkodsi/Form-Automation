...
/* The STUDY's own printed table comes first, not the HUD pull, and there are two
   ...
   The 150% test turns on this figure. Clinton Manor passes it by $12. A number
...
/* A permissions-locked PDF needs no password to open -- Preview shows one
   ...
   portfolio's properties file their studies this way, Noble Tower's current
...
/* THE LINES THE READER COULD NOT PLACE.
   ...
   rent is written into every row of that shape. Peterson Plaza's schedule came
...
/* Three spellings the real corpus uses that the first pass could not read, each
   ...
   "BR3"      puts the number AFTER the letters — Shiloh Village, 333 Holly and
   ...
   "2BR2BA"   runs the bath count straight on, so the \b after "br" never fires
              ...
              Plaza.
   "0 BEDROOM" is how Barnum House writes a studio. One property, but the reader
   ...
   Oaks on North Plaza, which are OCR misreads of a poor scan and not a naming
...
async function rsRuns(txt,ctx){ // content stream -> ctx.runs [{s,x,y,d}] in page space
  ...
  /* A gap the page draws by MOVING THE PEN rather than by printing spaces.
     ...
     the count and the next column, so 333 Holly and Shiloh Village read as ZERO
     unit types and their whole packages went unwritten. Their sibling Cornerstone
     studies, The Pines and Oaks on North Plaza, read fine only because Acrobat
     ...
     the suite caught it on Golden Link.
  ...
  const emit=str=>{if(!str)return;
    if(gap){gap=false;
      /* One boundary no width rule can judge: the inside of a printed figure.
         Golden Link draws "$1,580" as "$1," then a pen move then "580", and that
...
/* The form's own printed lines, by text rather than by position.
   ...
   Village and improved Oaks on North Plaza, and could not reach 333 Holly or The
   ...
   back from OCR as "Shiloh Village Apts. Part A Apartment Rents Show the actual".
...
/* ---- a printing that is NOT ours: Part A, read from the form's own table ----
   ...
   to 16: more than one printed row. On Market Square the implied shift walks
   ...
   characters are themselves a guess: Oaks on North Plaza's read $111,198 where
...
/* THE FLOOR, AND IT BELONGS TO ALL THREE TIERS.
   ...
   mistake, is one. The other is filed: Mapleview Towers' “HUD Rent Schedule -
   Mapleview Towers eff. 04.01.26.pdf” carries all 232 fields with 39 of them
...
/* ---- tier 2's premise, and the one pass that checks it -------------------
   ...
   reported success on all four. Oaks on North Plaza came back with the form's
   ...
   prints $91,922. Shiloh Village, 333 Holly and The Pines each did the same, and
...
function rsFillFromParsed(){const P=_rsUpload&&_rsUpload.parsed;if(!P)return;
  ...
  /* THE SCHEDULE OWNS THE ROSTER, SO THE STUDY MUST BE RE-READ AGAINST IT.
     ...
     moved Peterson Plaza's total contract rent by $143,950 between the two
...
function renderLauncher(){
  ...
  el('launcherBody').innerHTML=
    '<div class="lhead"><div class="lh-left"><div class="lh-name">'+esc(p.name)+(_showAl?'<span class="lh-alias">&ldquo;'+esc(_al)+'&rdquo;</span>':'')+'</div>'
      ...
      +'<div class="lh-right"><div class="lh-tools"><button class="txtbtn" id="pRename">Rename</button><span class="dotsep">&middot;</span><button class="txtbtn del" id="pDelete">Delete</button></div></div></div>'
...
/* One unusable source must not cost the whole package. Noble Tower's study is
