...
function dateEffCell(){
  ...
  return `<div class="field"><div class="flabel">Date rents will be effective</div>${_bad?'<div class="ucnote warn" style="margin:0 0 4px">\u26a0 That is not a date \u2014 enter it as mm/dd/yyyy.</div>':''}<div class="fbox uacell" data-box="${boxKey}" style="background:${c[1]};border-left-color:${c[0]}"><div class="uadrop" style="flex:1;min-width:0"><div class="uatrigger" tabindex="0">${lab}${srcLock('rent_schedule.date_eff_custom')}<span class="cvx">▾</span></div>${menu}</div>${ovIcons(['rent_schedule.date_eff_custom','rent_schedule.date_eff_source','rent_schedule.date_eff_rs'])}</div></div>`;}
...
/* Typing here does not edit the cell you can see: it switches the source to
  ...
  return '<div class="rbox uacell" data-box="'+boxKeyUA+'" style="background:'+c[1]+';border-left-color:'+c[0]+'"><div class="uadrop"><div class="uatrigger" tabindex="0"><span class="ualab">'+lab+'</span>'+srcLock('units.'+i+'.ua_custom')+'<span class="cvx">▾</span></div>'+menu+'</div></div>';}
...
function safmrBox(i){const src=get('units.'+i+'.safmr_source')||defSafmrSrc(i),hud=get('units.'+i+'.safmr_hud'),rcs=get('units.'+i+'.safmr_rcs'),custom=get('units.'+i+'.safmr_custom');
  ...
  return '<div class="rbox uacell" data-box="'+boxKeySA+'" style="background:'+c[1]+';border-left-color:'+c[0]+'"><div class="uadrop"><div class="uatrigger" tabindex="0"><span class="ualab">'+lab+'</span>'+srcLock('units.'+i+'.safmr_custom')+'<span class="cvx">▾</span></div>'+menu+'</div></div>';}
...
function ocafFactorCell(){const pub=get('ocaf.factor_pub'),fy=get('ocaf.factor_fy');
  ...
  return '<div class="rbox uacell" data-box="'+boxKey+'" style="background:'+c[1]+';border-left-color:'+c[0]+';max-width:330px;flex:0 1 auto"><div class="uadrop" style="flex:1;min-width:0"><div class="uatrigger" tabindex="0">'+lab+srcLock('ocaf.factor_custom')+'<span class="cvx">▾</span></div>'+menu+'</div>'+ovIcons(coupledKeys('ocaf.factor_custom'))+'</div>';}
...
/* The source padlock (provenance overhaul, Phase 1). A multi-source cell —
   utility allowance, 150% SAFMR, rents-effective date, OCAF factor — pulls its
   value from whichever source the hierarchy names (defUaSrc/defSafmrSrc/the RS
   year/the Federal Register pull) UNTIL a person deliberately picks one, which
   PINS the source key. The padlock makes that pin visible and reversible: a
   closed lock = you chose this source, fills keep it; an open, greyed lock =
   unlocked, the source follows the best available and a fill may move it.
   Reads `pinned` on the source key — the one invariant pullOcafFactor and the
   RS/RCS fills were made to stamp truthfully (auto = unpinned, a pick = pinned).
   Every path that can change that state runs a full renderBody, so the lock is
   only drawn here and never goes stale (FR-13). */
function srcLock(cusKey){const sp=srcSpec(cusKey);if(!sp)return '';
  const locked=!!(form[sp.srcKey]&&form[sp.srcKey].pinned);
  return '<span class="srclock'+(locked?' on':'')+'" data-lock="'+esc(cusKey)+'" role="button" tabindex="0" aria-pressed="'+(locked?'true':'false')+'" title="'+(locked?'Locked — you chose this source; fills keep it. Click to unlock.':'Unlocked — source follows the best available. Click to lock your choice.')+'" style="cursor:pointer;margin:0 0 0 1px;font-size:10.5px;line-height:1;flex:0 0 auto;opacity:'+(locked?'1':'.4')+'">'+(locked?'🔒':'🔓')+'</span>';}
...
function wireBody(){
  ...
  /* The source padlock: lock pins the currently-shown source (same as picking it
     from the dropdown); unlock clears the explicit pick and unpins, so the cell
     follows the hierarchy again and a fill may move it. stopPropagation keeps the
     click off the trigger that would open the menu. Same _pending/undo contract
     as the dropdown picks above (FR-17). */
  document.querySelectorAll('[data-lock]').forEach(o=>{const toggle=e=>{e.stopPropagation();e.preventDefault();
    const cusKey=o.getAttribute('data-lock');const sp=srcSpec(cusKey);if(!sp)return;
    const locked=!!(form[sp.srcKey]&&form[sp.srcKey].pinned);
    _pendingSnap=snapPend([sp.srcKey,sp.cusKey]);
    if(locked){form=store.editForm(form,sp.srcKey,'');if(form[sp.srcKey]){form[sp.srcKey].pinned=false;form[sp.srcKey].origin=null;}}
    else{srcSetSource(sp.cusKey,get(sp.srcKey)||sp.fallback);}
    _pending=[sp.srcKey];_refocusSel='[data-lock="'+cusKey+'"]';
    renderBody();setStatus(locked?'Source unlocked — a fill may move it to the best available source.':'Source locked — fills will keep your choice.');};
    o.addEventListener('click',toggle);
    o.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' ')toggle(e);});});
