...
function rcsFillFromParsed(){
  ...
  (P.units||[]).forEach(function(u,j){
    ...
    const ni=(UNITS.length?Math.max.apply(null,UNITS):-1)+1;
    const _a=rcsBaOf(u);
    setk('units.'+ni+'.br',_b);if(_a)setk('units.'+ni+'.ba',_a);
    if(u.count!=='')setk('units.'+ni+'.num_units',u.count);
    setk('units.'+ni+'.br_rcs',_b);if(_a)setk('units.'+ni+'.ba_rcs',_a);
    if(u.count!=='')setk('units.'+ni+'.num_rcs',u.count);
    if(u.proposed!=='')setk('units.'+ni+'.proposed',u.proposed);
    if(u.ua!=='')setk('units.'+ni+'.ua_rcs',u.ua);
    if(u.safmr!=='')setk('units.'+ni+'.safmr_rcs',u.safmr);
    UNITS.push(ni);added.push(u.type||_b);
  });
...
function wireBody(){
  ...
  document.querySelectorAll('[data-csclear]').forEach(x=>x.addEventListener('click',e=>{e.stopPropagation();e.preventDefault();const _cc=x.getAttribute('data-csclear');_pendingSnap=snapPend([_cc]);form=store.editForm(form,_cc,'');_pending=[_cc];_refocusSel='[data-trigfor="'+_cc+'"]';renderBody();setStatus('Cleared to blank.');}));document.querySelectorAll('[data-uaok]').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();const _rk='units.'+b.getAttribute('data-uaok')+'.ua_reviewed';pushCellUndo(_rk);form=store.editForm(form,_rk,'1');_pending=[_rk];renderBody();setStatus('UA conflict resolved — approved the shown value.');}));document.querySelectorAll('[data-safmropt]').forEach(o=>o.addEventListener('click',e=>{e.stopPropagation();const i=o.getAttribute('data-safmri'),v=o.getAttribute('data-safmropt');_pendingSnap=snapPend(['units.'+i+'.safmr_source','units.'+i+'.safmr_custom','units.'+i+'.safmr_reviewed']);srcSetSource('units.'+i+'.safmr_custom',v);if(safmrConflictOf(i))form=store.editForm(form,'units.'+i+'.safmr_reviewed','1');_pending=['units.'+i+'.safmr_source'];_refocusSel='[data-box="units.'+i+'.safmr_source"] .uatrigger';renderBody();setStatus('SAFMR source set.');}));
...
async function selftestSeed(){
  ...
  const pid=ps.length?ps[0].id:(await mpdb.createProperty('Sample Property Apartments')).id;
...
__rcsFill:()=>rcsFillFromParsed(),__UNITS:()=>UNITS.slice(),__moneySrcRows:(k)=>moneySrcRows(k),__rcsChecks:()=>rcsChecks(),__rcsTag:(k)=>rcsTag(k),__rcsFillKeys:()=>rcsFillKeys(),__rcsMatch:(i)=>rcsMatch(i),__rcsOf:(k)=>rcsOf(k),
