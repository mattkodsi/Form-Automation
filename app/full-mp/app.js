...
/* Which rent-adjustment programs this build offers. build-ra.py narrows this to
   ['rcs'] for the RCS-only release (OCAF/UAF disabled). Keep the default literal
   ['rcs','ocaf','uaf'] intact and unique — it is the build-ra.py patch anchor. */
let ENABLED_PROGRAMS=['rcs','ocaf','uaf'];
function progEnabled(p){return ENABLED_PROGRAMS.indexOf(p)>=0;}
...
let _undoStack=[]; let _undoNR=[]; let _undoLI=[]; let _undoPR=[]; let _pending=null,_refocusSel=null,_reopenBox=null,_pendingSnap=null; /* _reopenBox: after a render, re-open this cell's source menu — re-clicking the SELECTED option locks it, so keep the menu up to show the padlock */ let _rcsUpload=null; let _rcsBusy=null; let _rsUpload=null; let _rsArm=false;let _rsBusy=null;   // while set, the upload row shows what is being read
...
/* Flattened (2026-08-07): the utility allowance is ONE value now — units.N.ua_custom —
   filled schedule-first by the parses (see the fill), overridable from the dropdown or by
   typing, exactly like the contract rent beside it. No pointer, no shadow figures, no
   conflict to resolve: the schedule's figure and the study's are two rows in the cell's
   own dropdown, the same as the bedroom/bath and unit-count cells. The schedule-first
   DEFAULT (2026-08-10: the executed RS is the more reliable allowance) lives in the fill, not here. */
function uaResolvedOf(i){return numf(get('units.'+i+'.ua_custom'));}
/* A property with every utility owner-paid has a real $0 allowance. Testing
   >0 threw it away: the schedule's own $0 was unreadable and the RCS workbook's
   150% column came through blank. The file already knows the right test — has(). */
function uaHas(k){const v=get(k);return v!==''&&v!=null;}
function uaConflict(i){return false;}     // disagreement dropped — it is two dropdown rows now
function uaUnresolved(i){return false;}
/* Two 150% tests judge different figures — verified against the appraiser's filed
   survey. The PER-UNIT test (the chip in safmrNote and the over-ceiling counter)
   compares the proposed CONTRACT rent alone against that unit's 150% SAFMR: the
   survey's per-unit "As Is" table excludes the utility allowance. The AGGREGATE
   test (analysis()/computeAnalysis) compares the property's GROSS rent potential —
...
/* Flattened (2026-08-07): the 150% ceiling is ONE value now — units.N.safmr_custom — filled
   study-first (the study's printed table, then the HUD pull) by the RCS parse and the HUD fetch,
   overridable from the dropdown or by typing. No pointer, no shadow figures, no conflict: the study's
   figure and HUD's are the two rows of the cell's own dropdown. The 150% test reads this one value. */
function safmrResolvedOf(i){return numf(get('units.'+i+'.safmr_custom'));}
function safmrConflictOf(i){return false;}     // HUD vs study is two dropdown rows now, not a conflict
function safmrUnresolved(i){return false;}
...
/* Flattened (2026-08-07): the effective date is ONE value now — rent_schedule.date_eff_custom —
   filled from the executed schedule (+1yr) or typed. The RA tracker still OVERRIDES it: when the
   Related Affordable seam binds the date, date_eff_ra wins and the cell is locked (see dateEffCell).
   Legacy fallback (date_eff_rs / date_rents_effective) covers records saved before the flatten. */
function dateEffResolved(){const ra=get('rent_schedule.date_eff_ra');if(ra)return ra;
  return get('rent_schedule.date_eff_custom')||get('rent_schedule.date_eff_rs')||get('rent_schedule.date_rents_effective');}
/* The executed schedule's date + 1yr, offered to the dropdown live from the upload — the same way
   the allowance reads the schedule. So no stored date_eff_rs is needed on a fresh package. */
function dateEffRsOffer(){try{const p=_rsUpload&&_rsUpload.parsed;const d=p&&p.scalars&&p.scalars['rs_date'];if(!d)return null;const nx=rsYearOn(d);return nx?fmtDate(nx):null;}catch(e){return null;}}
function dateEffCell(){
  ...
  /* Flat now: one value + the one source dropdown, like every other cell. The value is
     rent_schedule.date_eff_custom; the schedule's date (+1yr) is the one document source,
     offered from moneySrcRows. Typing edits the value (data-date masks it); picking the
     schedule row writes it. The RA lock above still pre-empts the whole cell. */
  const k='rent_schedule.date_eff_custom';
  const v0=get(k)||get('rent_schedule.date_rents_effective');
  const val=/^\d{4}-\d{2}-\d{2}$/.test(String(v0||''))?fmtDate(v0):v0;
  const c=cellColor(k);
  const _bad=String(val||'').trim()!==''&&!dateParts(val);
  const input='<input type="text" class="dateeff-in" data-date="1" data-k="'+k+'" value="'+esc(val||'')+'" placeholder="mm/dd/yyyy" autocomplete="off">';
  return `<div class="field"><div class="flabel">Date rents will be effective</div>${_bad?'<div class="ucnote warn" style="margin:0 0 4px">\u26a0 That is not a date \u2014 enter it as mm/dd/yyyy.</div>':''}<div class="fbox" data-box="${k}" style="background:${c[1]};border-left-color:${c[0]}">${input}${srcTags(k)}${sourceMenu(k)}${ovIcons(k)}</div></div>`;
}
...
function pocCell(){const k='poc.name';const cur=get(k);const c=cellColors(k);
  return `<div class="field"><div class="flabel">Point of contact</div><div class="fbox poccell" data-box="${k}" style="background:${c[1]};border-left-color:${c[0]}"><input class="pocname-in" data-k="poc.name" value="${esc(cur)}" placeholder="Type a name, or pick a saved contact" autocomplete="off">${srcTags('poc.name')}${sourceMenu(k)}</div>${pocNote()}</div>`;}
...
function dirCell(f){const k=f.k;const P=DIR_PICK[k];const cur=get(k);
  ...
  return `<div class="field"><div class="flabel">${f.label}</div><div class="fbox poccell" data-box="${k}" style="background:${c[1]};border-left-color:${c[0]}">${pre}<input class="pocname-in" type="text" data-k="${k}" style="${nameTint}" value="${esc(cur)}" placeholder="Type a name, or pick a saved ${esc(P.one)}" autocomplete="off">${srcTags(k)}${sourceMenu(k)}</div>${dirNote(k)}</div>`;}
...
/* ── RA-LOCKED CELLS ──────────────────────────────────────────────────────
   ...
   the RA integrator's database answers for are not editable here.
...
function srcDisp(k,v){
  ...
  if(SRC_MONEY.test(k)||/\.(ua_source|safmr_source)$/.test(k)){const f=fmtMoney(v);return f?('$'+f):String(v);}   // money (incl. the UA/SAFMR source keys)
  ...
  return v;}
function srcOptRow(attrs,val,tag,sel,extra){const _c='uaopt srcopt'+(sel?' sel':'');return val?('<div class="'+_c+'" '+attrs+'>'+val+'<span class="uasub">'+esc(tag)+'</span>'+(extra||'')+'</div>'):('<div class="'+_c+' srcdim">\u2014<span class="uasub">'+esc(tag)+' \u00b7 not available</span></div>');}
/* The source lock (provenance overhaul, increment 2). A themed padlock \u2014 NOT an
   emoji \u2014 that rides the SELECTED option INSIDE the open dropdown (the menu is
   roomy; the cells are cramped), and ONLY when the cell is pinned: a person who
   picked a source or typed a custom value. Clicking it RELEASES \u2014 clears the
   explicit source so the hierarchy governs again and unpins, so a later fill or
   API pull can update the cell. This is what keeps a cell from being stuck on
   Custom, permanently un-overridable. Release is METADATA and is saved
   IMMEDIATELY, never through the pending/save-revert flow (see the handler). */
const LOCK_SVG='<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="4.5" y="10.5" width="15" height="10" rx="2"/><path d="M8 10.5V7a4 4 0 0 1 8 0v3.5"/></svg>';
function optLock(cusKey){const sp=srcSpec(cusKey);if(!sp)return '';
  if(!(form[sp.srcKey]&&form[sp.srcKey].pinned))return '';
  return '<span class="srclock" data-lock="'+esc(cusKey)+'" role="button" tabindex="0" aria-label="Release lock" title="Locked to your choice. Click to release so a fill or pull can update this cell.">'+LOCK_SVG+'</span>';}
...
function moneySrcRows(k){
  ...
  if((m=k.match(/^units\.(\d+)\.ua_custom$/)))return [{tag:'Executed RS',val:uaExecOffer(+m[1])},{tag:'RCS report',val:rcsUnitVal(+m[1],'ua')}];   // UA flattened: the schedule (default) then the study — RS-first; $0-safe via uaExecOffer
  if((m=k.match(/^units\.(\d+)\.safmr_custom$/)))return [{tag:'RCS report',val:rcsUnitVal(+m[1],'safmr')},{tag:'HUD API',val:safmrHudOffer(+m[1])}];   // SAFMR flattened: the study's ceiling (default) then the live HUD pull
  if(k==='rent_schedule.date_eff_custom')return [{tag:'Executed RS + 1 yr',val:dateEffRsOffer()}];   // date flattened: the schedule's date +1yr, offered live from the upload
  if(k==='ocaf.factor_custom'){const pub=get('ocaf.factor_pub');return [{tag:'Federal Register',val:pub?String(pub):null}];}   // OCAF flattened: the published factor is the one source (stored from the FR pull); the FY shows in the OCAF header
...
function sigTitleCell(f){const c=cellColors('sig.title');const pk='sig.principal';const pc=cellColors(pk);
  ...
  return `<div class="fpair sigpair"><div class="field"><div class="flabel">${f.label}</div><div class="fbox" data-box="sig.title" style="background:${c[1]};border-left-color:${c[0]}"><input type="text" data-k="sig.title" value="${esc(get('sig.title'))}" autocomplete="off">${srcTags('sig.title')}${sourceMenu('sig.title')}</div>${ovNote('sig.title')}</div><div class="ofthe">of the</div><div class="field"><div class="flabel">Principal</div><div class="fbox seldrop" data-box="${pk}" style="background:${pc[1]};border-left-color:${pc[0]}">${dd}</div>${ovNote(pk)}</div></div>`;}
...
function selectCell(f){const c=cellColors(f.k);const _et=f.k==='owner.entity_type';
  ...
  if(f.k==='owner.entity_type'&&get(f.k)==='Other (specify)'){const ok='owner.entity_type_other';const oc=cellColors(ok);
    const other=`<div class="field"><div class="flabel">Specify entity type</div><div class="fbox" data-box="${ok}" style="background:${oc[1]};border-left-color:${oc[0]}"><input type="text" data-k="${ok}" value="${esc(get(ok))}" placeholder="e.g. Limited Liability Company" autocomplete="off">${sourceMenu(ok)}</div>${ovNote(ok)}</div>`;
...
function mgmtCell(){const src=get('tenant.mgmt_source')||'property';const propHas=ADDR.some(k=>get(k)!=='');
  ...
  const propRow='<div class="uaopt srcopt'+(src==='property'?' sel':'')+'" data-mgmt="property">'+(propHas?esc(pretty):'Property address')+'<span class="uasub">'+(propHas?'Property address':'Set it in Section 2')+'</span></div>';
  const menu='<div class="uamenu">'+propRow+'<div class="uadiv" aria-hidden="true"></div><div class="uaopt'+(src==='custom'?' sel':'')+'" data-mgmt="custom">Custom\u2026</div></div>';
...
function renderPrincipals(){
  const rows=PRINCIPALS.map(i=>{const nk='principals.'+i+'.name',tk='principals.'+i+'.title';const nc=cellColors(nk),tc=cellColors(tk);
    return `<div class="prinrow"><div class="field"><div class="fbox" data-box="${nk}" style="background:${nc[1]};border-left-color:${nc[0]}"><input type="text" data-k="${nk}" value="${esc(get(nk))}" autocomplete="off">${rsTag(nk)}${sourceMenu(nk,[{tag:'Executed RS',val:rsPrin(i,'name')}])}</div>${ovNote(nk)}</div><div class="field"><div class="fbox" data-box="${tk}" style="background:${tc[1]};border-left-color:${tc[0]}"><input type="text" data-k="${tk}" value="${esc(get(tk))}" autocomplete="off">${rsTag(tk)}${sourceMenu(tk,[{tag:'Executed RS',val:rsPrin(i,'title')}])}</div>${ovNote(tk)}</div><div class="urx">${PRINCIPALS.length>1?`<button class="trash" data-delprin="${i}" title="Delete">\u2715</button>`:''}</div></div>`;}).join('');
...
const TAG_ORIGIN={'Executed RS':'rs','Executed RS + 1 yr':'rs','RCS report':'rcs','Related Affordable':'ra','HUD API':'hud','Federal Register':'fr'};   // a per-cell pull's source tag -> the origin cellColor reads
function cellState(key){
  ...
  if(sp){                                     // a source-family cell: reproduce the family painter (FR-4)
    ...
    const _cs=srcCellState(sp.cusKey);         // judged on the RESOLVED figure vs on file
    if(src==='custom')return _cs==='overridden'?'overridden':(_cs==='database'?'database':'new');
    const fk=sp.figKey||(()=>null);
    const anyVal=(sp.names||[]).some(n=>numf(get(fk(n)))>0)||numf(get(sp.cusKey))>0;
    let state=anyVal?srcOf(fk(src)):'new';     // the chosen figure's own saved-state
    if(_cs==='overridden')state='overridden';else if(_cs==='database'&&state==='new')state='database';
    if((sp.unresolved&&sp.unresolved())||srcOf(sp.srcKey)==='overridden')state='overridden';   // conflict-amber + switched-source
    return state;
  }
  const c=form[key]; if(!c)return 'new';
  const v=String(c.value==null?'':c.value);
  const db=c.db_value;
  if(db!=null)return v===String(db)?'database':'overridden';   // saved, incl. saved-empty: blue if unchanged, amber once an entry differs (FR cell-model)
...
// Contact cells (POC / appraiser / CA / signatory names) render the SAME dropdown
// as every other source cell: document/platform rows on top (they fill the whole
// section), then every saved contact, then a Custom line. Picking a contact or a
// doc pins the cell; the padlock releases it. One component, one handler.
function isContactCell(key){return key==='poc.name'||(typeof DIR_PICK!=='undefined'&&!!DIR_PICK[key]);}
function contactLines(key){
  const cur=String(get(key)==null?'':get(key));const org=form[key]&&form[key].origin;
  const eqn=(x)=>x!=null&&String(x).trim().toLowerCase()===cur.trim().toLowerCase();
  const out=[];
  if(key==='poc.name'){
    const ra=raVal('poc.name');const rm=pocMatchRcs().name;
    out.push({kind:'doc',src:'cdoc:ra',tag:'Related Affordable',val:(ra==null||ra==='')?null:ra,sel:org==='ra'&&eqn(ra)});
    out.push({kind:'doc',src:'cdoc:rcs',tag:'RCS report',val:(rm==null||rm==='')?null:rm,sel:org==='rcs'&&eqn(rm)});
  }else{
    const dr=DIR_SRCROW[key];
    if(dr){const v=dr.val();const ori=TAG_ORIGIN[dr.tag]||'doc';out.push({kind:'doc',src:'cdoc:doc',tag:dr.tag,val:(v==null||v==='')?null:v,sel:org===ori&&eqn(v)});}
  }
  const list=(key==='poc.name')?(mpdb?mpdb.listContacts():[]):(DIR_PICK[key]?dirList(DIR_PICK[key].kind):[]);
  const subf=(ct)=>(key==='poc.name')?(ct.email||''):(DIR_PICK[key]?DIR_PICK[key].sub(ct):'');
  (list||[]).forEach(ct=>out.push({kind:'contact',src:'cpick:'+ct.id,tag:ct.name,val:subf(ct),sel:org==='contact'&&eqn(ct.name)}));
  out.push({kind:'free',src:'custom',tag:'Custom',val:cur,sel:org==='typed'});
  return out;
}
function sourcesFor(key){
  const sp=(typeof srcSpecAny==='function')?srcSpecAny(key):null;
  if(sp){                                     // Engine A: resolver + shadow figures (UA/SAFMR/date/OCAF)
  ...
  // Engine B: table-driven pull cells — text via SRCPICK_ROWS, money via moneySrcRows.
  // Selection is the cell's own origin (no resolver key); Custom is a typed value.
  let brows=[];
  if(typeof SRCPICK_ROWS!=='undefined'&&SRCPICK_ROWS[key])brows=SRCPICK_ROWS[key]();
  else if(typeof moneySrcRows==='function'){const mr=moneySrcRows(key);if(mr&&mr.length)brows=mr;}
  if(brows.length)return srcLines(key,brows);
  if(isContactCell(key))return contactLines(key);
  return [];   // kind E — no dropdown; or a family not yet folded in
}
/* sourceMenu(key): THE ONE dropdown — the ideal (UA/SAFMR) menu rendered from
   sourcesFor for any cell. Sources (dim when empty, FR-1), always a Custom line
   for open cells, a padlock on the selected line when the cell is pinned. Every
   option is keyed data-srcpick=cell / data-srcopt=source so ONE handler serves
   all line-kinds. Pure + unwired — the wiring replaces srcPick / the *opt menus. */
/* srcLines(key,rows): map explicit [{tag,val}] rows to typed lines (origin-based
   selection + a trailing Custom) — shared by sourcesFor's Engine-B branch and by
   sourceMenu callers that pass their own inline rows (principals, count cells). */
function srcLines(key,rows){const cur=form[key];const org=cur&&cur.origin;
  const ls=rows.map(r=>{const oc=TAG_ORIGIN[r.tag];return {kind:'doc',src:oc||r.tag,tag:r.tag,val:(r.val==null||r.val==='')?null:r.val,sel:!!oc&&org===oc};});
  ls.push({kind:'free',src:'custom',tag:'Custom',val:cur?cur.value:'',sel:org==='typed'});return ls;}
function sourceOpts(key,lines){
  const pinned=!!(form[key]&&form[key].pinned);
  const lock=(sel)=>(sel&&pinned)?('<span class="srclock" data-lock="'+esc(key)+'" role="button" tabindex="0" aria-label="Release lock" title="Locked to your choice. Click to release so a fill or pull can update this cell.">'+LOCK_SVG+'</span>'):'';
  return lines.map(l=>{
    const attrs='data-srcpick="'+esc(key)+'" data-srcopt="'+esc(l.src)+'"';
    if(l.kind==='contact')return '<div class="uaopt'+(l.sel?' sel':'')+'" '+attrs+'>'+esc(l.tag)+(l.val?'<span class="uasub">'+esc(l.val)+'</span>':'')+lock(l.sel)+'</div>';
    if(l.kind==='free')return '<div class="uaopt'+(l.sel?' sel':'')+'" '+attrs+'>Custom\u2026'+lock(l.sel)+'</div>';
    return l.val!=null
      ?'<div class="uaopt srcopt'+(l.sel?' sel':'')+'" '+attrs+' data-srcv="'+esc(l.val)+'">'+esc(srcDisp(key,l.val))+'<span class="uasub">'+esc(l.tag)+'</span>'+lock(l.sel)+'</div>'
      :'<div class="uaopt srcopt srcdim" '+attrs+'>\u2014<span class="uasub">'+esc(l.tag)+' \u00b7 not available</span></div>';
  }).join('');
}
function sourceMenu(key,rows){
  const lines=rows?srcLines(key,rows):sourcesFor(key); if(!lines.length)return '';
  const pinned=!!(form[key]&&form[key].pinned);
  const lock=(sel)=>(sel&&pinned)?('<span class="srclock" data-lock="'+esc(key)+'" role="button" tabindex="0" aria-label="Release lock" title="Locked to your choice. Click to release so a fill or pull can update this cell.">'+LOCK_SVG+'</span>'):'';
  const opts=sourceOpts(key,lines);
...
function numBox(k,ph,noIcons){const c=boxColor(k);
  ...
  return `<div class="rbox" data-box="${k}" style="background:${c[1]};border-left-color:${c[0]}"><input type="text"${_cnt?' data-money="1"':''} data-k="${k}" value="${esc(_cnt?fmtMoney(get(k)):get(k))}" placeholder="${esc(ph||'')}">${rsTag(k)}${_rsCell?sourceMenu(k,[{tag:'Executed RS',val:_rv}]):''}${noIcons?'':ovIcons(k)}</div>`;}
...
function uaCellColors(i){return cellColor('units.'+i+'.ua_custom');}   // flat: one key, the one colour rule — like every plain money cell
function safmrCellColors(i){return cellColor('units.'+i+'.safmr_custom');}   // flat: one key, the one colour rule
/* Legacy migration (2026-08-07): a package saved before a cell was flattened holds its old shadow
   keys (ua_exec/ua_rcs/ua_source; safmr_rcs/safmr_hud/safmr_source) and an empty *_custom. Seed the
   one value from the old study-first resolution, as DATABASE-state so the loaded package reads as on
   file (not unsaved) and its next real save persists *_custom for good. Idempotent: fires only where
   *_custom is empty and a legacy figure is present. Runs in the load paths, after deriveUnits, before
   snapForm. */
function migrateFlatCells(){const has=v=>v!==''&&v!=null;
  const seed=(ck,v,org,saKeys)=>{const sa=saKeys.map(k=>form[k]&&form[k].saved_at).find(Boolean)||null;
    form[ck]={value:String(v),source:'database',saved_at:sa,prior_value:null,prior_source:null,db_value:String(v),origin:org,pinned:false};};
  (UNITS||[]).forEach(function(i){
    /* Utility allowance: schedule (ua_exec) then study (ua_rcs) — RS-first, matching db.js. */
    { const ck='units.'+i+'.ua_custom',cc=form[ck];
      if(!(cc&&has(cc.value))){const ex=get('units.'+i+'.ua_exec'),rc=get('units.'+i+'.ua_rcs'),sr=get('units.'+i+'.ua_source');
        if(has(ex)||has(rc)){const src=sr||(has(ex)?'exec':'rcs');const v=src==='rcs'?rc:ex;
          if(has(v))seed(ck,v,src==='rcs'?'rcs':'rs',['units.'+i+'.ua_exec','units.'+i+'.ua_rcs']);}} }
    /* 150% SAFMR: study (safmr_rcs) then HUD (safmr_hud). */
    { const ck='units.'+i+'.safmr_custom',cc=form[ck];
      if(!(cc&&has(cc.value))){const hd=get('units.'+i+'.safmr_hud'),rc=get('units.'+i+'.safmr_rcs'),sr=get('units.'+i+'.safmr_source');
        if(has(hd)||has(rc)){const src=sr||(has(rc)?'rcs':'hud');const v=src==='rcs'?rc:hd;
          if(has(v))seed(ck,v,src==='rcs'?'rcs':'hud',['units.'+i+'.safmr_rcs','units.'+i+'.safmr_hud']);}} }
  });
  /* Effective date: the schedule's date (+1yr), or the old typed custom (already in date_eff_custom).
     The RA lock (date_eff_ra) is separate and pre-empts the cell regardless. */
  { const ck='rent_schedule.date_eff_custom',cc=form[ck];
    if(!(cc&&has(cc.value))){const v=get('rent_schedule.date_eff_rs')||get('rent_schedule.date_rents_effective');
      if(has(v))seed(ck,v,'rs',['rent_schedule.date_eff_rs','rent_schedule.date_rents_effective']);}}
  /* OCAF factor: the Federal Register value (factor_pub), seeded into the one flat value for a
     legacy record whose FR pull predates the flatten. No cross-cycle carry — the factor is pulled
     fresh each OCAF year — so this only lifts a value already stored on THIS record. */
  { const ck='ocaf.factor_custom',cc=form[ck];
    if(!(cc&&has(cc.value))){const v=get('ocaf.factor_pub');
      if(has(v))seed(ck,v,'fr',['ocaf.factor_pub']);}}
}
/* The executed schedule's allowance, offered to the dropdown $0-safe. rsUnit drops
   a real 0 (an owner-paid unit's $0 is a value, not a blank), so the cell reads the
   parse directly here — the same $0-safe rule the resolver's uaHas already keeps. */
function uaExecOffer(i){try{const u=_rsUpload&&_rsUpload.parsed&&_rsUpload.parsed.units;const r=u&&u[i];const v=r&&r.ua;return (v==null||v==='')?null:String(v);}catch(e){return null;}}
/* A plain money cell now, identical in shape to the contract rent beside it. Its two
   source rows — the study (the default) and the schedule — come from moneySrcRows, and
   ovIcons is added by the row (col), so noIcons is set. */
function uaBox(i){return moneyBox('units.'+i+'.ua_custom',1);}
function uaNoteCell(i){return '';}   // the study's reading lives in the cell's dropdown now; a difference is not a thing to resolve before the row can be saved
...
function unitCountCell(i){const k='units.'+i+'.num_units';const c=cellColors(k);const rv=rsUnit(i,'count');
  ...
  return `<div class="rbox" data-box="${k}" style="background:${c[1]};border-left-color:${c[0]}"><input type="text" data-money="1" data-k="${k}" value="${esc(fmtMoney(get(k)))}">${srcTags(k)}${sourceMenu(k,[{tag:'Executed RS',val:rv},{tag:'RCS report',val:cv}])}</div>`;}
...
/* HUD's 150% ceiling for this unit, offered to the dropdown live from the last HUD pull
   (_hud.data), the same way uaExecOffer reads _rsUpload — so a stored safmr_hud is no longer
   needed. Null when no pull has landed or the unit has no bedroom count yet. */
function safmrHudOffer(i){try{if(!_hud||!_hud.data)return null;const rents=_hud.data.zip_rents||_hud.data.area_rents;if(!rents)return null;const br=get('units.'+i+'.br');if(!br)return null;const v=hudCeil(rents,br);return (v==null||v==='')?null:String(v);}catch(e){return null;}}
function safmrBox(i){return moneyBox('units.'+i+'.safmr_custom',1);}   // a plain money cell now — its two sources (study, HUD) come from moneySrcRows
function safmrNote(i){const res=safmrResolvedOf(i);
  /* No conflict branch — the study's ceiling and HUD's are two dropdown rows now. This note is
     only the 150% test READOUT (pass/over) and the "needed for the 150% test" prompt. */
...
function refreshFlags(){const _si=sectionIssues();
...
function applyHudSafmr(){if(!_hud.data)return 0;const rents=_hud.data.zip_rents||_hud.data.area_rents;if(!rents)return 0;
  /* Flattened: the HUD ceiling fills the one SAFMR value — but the STUDY wins. The RCS parse
     writes safmr_custom with origin 'rcs' and leaves it unpinned; this defers to that, and to any
     figure the reader typed or picked (pinned). So HUD fills only the cells the study has not, and
     a later study fill overwrites a HUD one whichever order they arrive. Runs async + on every ZIP/
     br change + on form open, so it stays idempotent: write only when the value actually differs. */
  let changed=0;UNITS.forEach(i=>{const br=get('units.'+i+'.br');if(!br)return;const v=hudCeil(rents,br);
    if(!v)return;const ck='units.'+i+'.safmr_custom',cc=form[ck];
    if(cc&&((cc.origin==='rcs'&&!cc.pinned)||cc.pinned))return;   // study-first, and never stomp a typed/picked figure
    if(get(ck)!==v){form=store.editForm(form,ck,v);markCycle(ck);if(form[ck]){form[ck].origin='hud';form[ck].fromParse=true;form[ck].pinned=false;}changed++;}});
  return changed;}
...
function rsOf(k){
  ...
  m=k.match(/^units\.(\d+)\.ua_custom$/);             if(m)return uaExecOffer(+m[1]);   // the schedule's allowance is the UA cell's RS side now ($0-safe)
...
function rsTag(k){
  ...
  const num=/^(units|ns8|nonrev)\.\d+\.(current|proposed|num_units|ua_custom|rent|avg_rent)$/.test(k);
...
/* The study is addressed to a person: "Mr. Jamie Rivera" above the subject
...
function rcsOf(k){
  ...
  m=k.match(/^units\.(\d+)\.ua_custom$/);       if(m)return rcsUnitVal(+m[1],'ua');   // the study's allowance is the UA cell's RCS side now
  m=k.match(/^units\.(\d+)\.safmr_rcs$/);      if(m)return rcsUnitVal(+m[1],'safmr');
  m=k.match(/^units\.(\d+)\.safmr_custom$/);   if(m)return rcsUnitVal(+m[1],'safmr');   // the study's ceiling is the SAFMR cell's RCS side now
...
function rcsTag(k){
  ...
  const num=/^(units\.\d+\.(proposed|ua_rcs|ua_custom|safmr_rcs|safmr_custom|num_rcs)|nonrev\.\d+\.rent)$/.test(k);
...
function rcsFillKeys(){
  ...
  (UNITS||[]).forEach(function(i){['proposed','ua_custom','safmr_custom','br_rcs','ba_rcs','num_rcs'].forEach(function(f){ks.push('units.'+i+'.'+f);});});
...
function rcsFillFromParsed(opts){
  ...
  const _wrote=[],_skip=[];
  const mark=k=>{markCycle(k);if(form[k]){form[k].fromParse=true;form[k].origin='rcs';form[k].pinned=false;}if(_wrote.indexOf(k)<0)_wrote.push(k);};   // came from the study — origin 'rcs', and a bulk fill pins nothing
  /* A deliberate entry is pinned; a re-read of the study LEAVES it and reports it
     rather than stomping it. Identical value = no conflict, left silently. Apply-
     over passes force. Same rule as the schedule fill — see rsFillFromParsed. */
  const put=(k,v)=>{if(v==null||v==='')return;
    const cur=form[k];
    // Only a non-empty deliberate entry is protected — see rsFillFromParsed.
    if(cur&&cur.pinned&&cur.value!==''&&cur.value!=null&&!_force){if(String(cur.value)!==String(v)&&_skip.indexOf(k)<0)_skip.push(k);return;}
    form=store.editForm(form,k,String(v));mark(k);};
  ...
  (UNITS||[]).forEach(function(i){
    ...
    { const _uc=form['units.'+i+'.ua_custom']; if(!(_uc&&(_uc.pinned||_uc.origin==='rs'))&&m.u.ua!==''&&m.u.ua!=null)put('units.'+i+'.ua_custom',m.u.ua); }   // schedule-first for UA (2026-08-10): the study fills only where the schedule has not; put still protects a pinned figure; $0 is a real allowance
    if(m.u.safmr!==''&&m.u.safmr!=null)put('units.'+i+'.safmr_custom',m.u.safmr);   // the study's 150% ceiling IS the value (study-first) — put so it wins over the HUD pull
  ...
  (P.units||[]).forEach(function(u,j){
    ...
    { const _uc=form['units.'+ni+'.ua_custom']; if(!(_uc&&(_uc.pinned||_uc.origin==='rs'))&&u.ua!==''&&u.ua!=null)put('units.'+ni+'.ua_custom',u.ua); }   // schedule-first for UA: study fills only where the schedule has not
    if(u.safmr!==''&&u.safmr!=null)put('units.'+ni+'.safmr_custom',u.safmr);
  ...
  _rcsFill=fillRecord(_rcsUpload,_wrote,_skip);rcsRemember();
...
function rsFillFromParsed(opts){const P=_rsUpload&&_rsUpload.parsed;if(!P)return;
  const _force=!!(opts&&opts.force);
  const _wrote=[],_skip=[];
  const mark=k=>{markCycle(k);if(form[k]){form[k].fromParse=true;form[k].origin='rs';form[k].pinned=false;}if(_wrote.indexOf(k)<0)_wrote.push(k);};   // came from the schedule, not typed — origin 'rs', and a bulk fill pins nothing (editForm's typed/pinned default is overridden here). Tags an override as "parsed", not just "changed", in its note text
  /* A deliberate entry — a value the person typed or a source they picked — is
     pinned. A re-fill LEAVES it and reports it rather than stomping a figure that
     already went to HUD; an identical value is no conflict, so it is left silently.
     "Apply over my entries" re-runs with force and overwrites. See fillNote. */
  const setk=(k,v)=>{if(raLockedKey(k))return;if(v==null||v==='')return;
    const cur=form[k];
    // Protect only a deliberate, NON-EMPTY entry. A cleared cell is pinned too
    // (editForm pins every write), but an empty cell is not a value worth keeping
    // from a document — let the fill populate it.
    if(cur&&cur.pinned&&cur.value!==''&&cur.value!=null&&!_force){if(String(cur.value)!==String(v)&&_skip.indexOf(k)<0)_skip.push(k);return;}
    form=store.editForm(form,k,String(v));mark(k);};
  ...
  if(P.scalars['rs_date']){const nx=rsYearOn(P.scalars['rs_date']);
    if(nx){
      // Flattened: the schedule's date (+1yr) IS the value unless a deliberate one is pinned. mark() sets
      // origin 'rs'; the dropdown offer reads the schedule live (dateEffRsOffer), so no date_eff_rs is stored.
      const dk='rent_schedule.date_eff_custom',dc=form[dk];
      if(!(dc&&dc.pinned&&dc.value!==''&&dc.value!=null)){form=store.editForm(form,dk,nx);mark(dk);}}}
  ...
  P.units.forEach((u,ix)=>{const bb=rsParseUnitType(u.type);setk('units.'+ix+'.br',bb.br);setk('units.'+ix+'.ba',bb.ba);setk('units.'+ix+'.label',bb.label);
    ...
    /* Schedule-first for the UA (2026-08-10): the executed rent schedule is the more
       reliable allowance. Any UAF adjustment made between this schedule and the study
       also flows through this app and is validated there, so the study must not
       outrank the schedule for UA. The schedule writes it here (setk still protects a
       pinned figure); the RCS fill defers to an rs-origin cell. Order cannot decide it
       — whichever file arrives first, the schedule holds the cell once it has filled
       it. $0 is a real allowance (owner-paid utilities), so no >0 guard. */
    { const _uak='units.'+ix+'.ua_custom';
      if(u.ua!==''&&u.ua!=null)setk(_uak,u.ua); }});
  ...
  _rsFill=fillRecord(_rsUpload,_wrote,_skip);rsRemember();
  deriveUnits();
  ...
  if(_rcsFill&&_rcsUpload&&_rcsUpload.parsed&&_rcsFill.name===(_rcsUpload.name||'')){
    rcsFillFromParsed(opts);   // renders, and reports its own count (force propagates)
...
function fillRecord(up,keys,skipped){return up?{name:up.name||'',n:keys.length,keys:keys.slice(),skipped:(skipped||[]).slice()}:null;}
...
function fillNote(rec,up){
  ...
  const base=left?('Filled '+n+' \u2014 '+left+' still to save.'):('Filled '+n+', all saved.');
  /* What the fill LEFT ALONE is part of the reading: a re-fill skips the values a
     person deliberately entered, and says so, so a protected figure never looks
     like it was quietly overwritten. The Apply-over button (below) reverses it. */
  const sk=(rec.skipped||[]).filter(k=>form[k]&&form[k].pinned).length;
  return sk?(base+' Left '+sk+' you\u2019d entered.'):base;}
/* Offered only when a re-fill actually skipped something the user still has
   pinned \u2014 one click re-runs the fill with force and overwrites those cells. */
function applyOverBtn(rec,up,id){
  const sk=(rec&&up&&rec.name===(up.name||'')&&rec.skipped)?rec.skipped.filter(k=>form[k]&&form[k].pinned).length:0;
  return sk?('<button class="btn sm" id="'+id+'" title="Overwrite the '+sk+' value'+(sk===1?'':'s')+' you entered with this document\u2019s">Apply over my '+sk+' entr'+(sk===1?'y':'ies')+'</button>'):'';}
...
function renderSources(){
  ...
  const rcs=_rcsBusy
    ...
    ?srcTile({icon:OK,name:up.name,
              ...
              acts:(_rn?'<button class="btn sm teal" id="rcsApply">Fill form from study</button>'+applyOverBtn(_rcsFill,up,'rcsApplyOver'):'')
  ...
  if(_rsBusy)rs=srcTile({icon:SPIN,name:_rsBusy.name||'Rent schedule',
    ...
    rs=srcTile({icon:_half?WARN:OK,name:ru.name,
      ...
      acts:'<button class="btn sm teal" id="rsApply">Fill form from RS</button>'+applyOverBtn(_rsFill,ru,'rsApplyOver')+'<button class="btn sm" id="upRs">Replace</button>'});}
...
function sectionKeys(n){if(n===10)return ['ocaf.g','ocaf.rate_type','ocaf.ds_annual','ocaf.ds_t12','ocaf.ds_f12','ocaf.factor_pub','ocaf.factor_custom'];
  ...
  /* `label` belongs here. It is the unit type's own name — plainly content in
     ...
     still be told the section was not started. ua_custom is the flattened UA cell. */
  if(n===6)return UNITS.flatMap(i=>['units.'+i+'.br','units.'+i+'.ba','units.'+i+'.label','units.'+i+'.num_units','units.'+i+'.current','units.'+i+'.proposed','units.'+i+'.ua_custom','units.'+i+'.safmr_custom']);const fs=FIELD_SECTIONS.find(s=>s.n===n);return fs?fs.fields.flatMap(f=>f.type==='sigtitle'?['sig.title','sig.principal']:f.type==='pair'?f.items.map(x=>x.k):f.type==='addr'?ADDR:(f.type==='caaddr'?CA_ADDR:(f.type==='appraddr'?APPR_ADDR:(f.type==='mgmtaddr'?MGMT_ADDR:(f.prefix?[f.prefix,f.k]:[f.k]))))):[];}
...
/* Every requirement the package's own scoring knows about, grouped by the
   section that holds it — gate 1's identity keys plus each document's own.
   score.js already tags every one with a `sec`; nothing here decides what is
   required, it only asks. */
/* Both halves of "what is wrong here", from ONE packageScore(). Split because
   they mean different things to a reader: `miss` stops a document being
   written, `cav` does not — it changes what the document SAYS. The management
   street is the clearest case (score.js:167): the tenant notice generates
   without it and simply never tells the tenant where to inspect the materials.
   Returned together so the rail can compute both marks off a single score
   rather than scoring the package once per section. */
function sectionIssues(){
  const miss={},cav={};
  try{const S=packageScore();
    const add=(m,into)=>{(m||[]).forEach(x=>{if(x&&x.sec!=null)(into[x.sec]=into[x.sec]||[]).push(x);});};
    add(S&&S.profile&&S.profile.missing,miss);   // gate 1: is this a real property record
    add(S&&S.blockers,miss);                     // gate 2: can each document be written
    ((S&&S.docs)||[]).forEach(d=>add(d.missing,miss));
    add(S&&S.caveats,cav);                       // gate 3: will it print complete
  }catch(e){}
  return {miss:miss,cav:cav};}
/* An indicator computes; it never asserts (FORM-RULES 15). This used to return
   'confirmed' for any section that merely held SOMETHING and had nothing
   overridden — so a package one click old, carrying only the property name the
   record supplied, read 9 of 10 confirmed with green ticks down the rail while
   not one document could be built. "Confirmed" now means the package's own
   scoring finds nothing missing in that section, which is the same computation
   the ring and the six-document card have used all along. */
/* `_si` is passed in by the two renderers that mark every section at once, so
   a repaint scores the package ONCE instead of once per section. */
function sectionStatus(n,_si){
  _si=_si||sectionIssues();
  /* Confirmed, or confirmed-with-a-note. Every path that used to return 'ok'
     asks this instead, so a caveat cannot reach the rail as a full tick through
...
/* ONE place decides how a section's mark looks, because two decided before and
   ...
   "confirmed", because it collapsed everything that was not 'warn' to 'ok'. So
   typing anywhere turned every untouched section green, and the rail beside it
   went on showing them empty. That is FORM-RULES 13 exactly: a live re-render
   must draw what the full render draws, and the way to guarantee it is to have
   one description rather than two implementations. */
const PILL_LOOK={empty:['pill empty','not started'],warn:['pill warn','review'],
                 cav:['pill ok cav','confirmed'],ok:['pill ok','confirmed']};
/* Why the tick is dimmed rather than turned amber: the section IS ready — every
   document it feeds can be written. Painting it the same colour as a section
   that cannot generate at all would flatten a distinction that is real. */
...
function pullBtnState(api){
  ...
  let done=false;
  if(api==='safmr')done=UNITS.some(i=>numf(safmrHudOffer(i))>0);   // the HUD pull has landed when its live offer resolves (flattened: no stored safmr_hud)
...
/* Flattened (2026-08-07): the OCAF factor is ONE value now — ocaf.factor_custom — the Federal
   Register pull fills it (FR-first) or the user types it. factor_pub + fy/pubdate/state stay as the
   pull's provenance (the dropdown offer + the "already current" check), coupled to the value. */
function ocafFactorResolved(){return numf(get('ocaf.factor_custom'));}
...
function ocafFactorCell(){const k='ocaf.factor_custom';const c=cellColor(k);
  /* Flat now: one value (the factor %) + the one source dropdown, like every other cell. The Federal
     Register figure is the one document source (from moneySrcRows); the pull's FY/date/state ride
     with the value via coupledKeys. Plain data-k input (no money mask) so 1.042 keeps its decimals. */
  const input='<input type="text" data-k="'+k+'" value="'+esc(get(k)||'')+'" placeholder="4.9" style="flex:1;min-width:0"><span class="cur" style="padding:0 8px 0 2px">%</span>';
  return '<div class="rbox money" data-box="'+k+'" style="background:'+c[1]+';border-left-color:'+c[0]+';max-width:330px;flex:0 1 auto">'+input+srcTags(k)+sourceMenu(k)+ovIcons(coupledKeys(k))+'</div>';}
...
async function pullOcafFactor(opts){opts=opts||{};const auto=!!opts.auto;
  ...
  if(auto){const _fc=form['ocaf.factor_custom'];if(_fc&&_fc.pinned&&_fc.value!==''&&_fc.value!=null)return; // a deliberately-pinned factor opts out of the auto-pull (flattened: was factor_src==='custom')
  ...
  try{const r=await supaClient.functions.invoke('ocaf-factor',{body:{year:effYear()||undefined}});
    ...
    form=store.editForm(form,'ocaf.factor_pub',String(v));form=store.editForm(form,'ocaf.factor_fy',String(d.fy||''));form=store.editForm(form,'ocaf.factor_pubdate',String(d.publication_date||''));form=store.editForm(form,'ocaf.factor_state',usedNat?'US':stt);
    /* Flattened: the published figure fills the one value (factor_custom) unless a
       deliberate factor is pinned. factor_pub/fy/pubdate/state stay as the dropdown
       offer + provenance that rides with the value. A Federal Register pull is the
       hierarchy's default source, not a user lock, so stamp the keys we write the way
       the RS/RCS fills stamp theirs: origin 'fr', pinned false — otherwise an
       auto-pulled factor would read as locked and the source padlock would lie. */
    const _fc=form['ocaf.factor_custom'],_wroteFc=!(_fc&&_fc.pinned&&_fc.value!==''&&_fc.value!=null);
    if(_wroteFc)form=store.editForm(form,'ocaf.factor_custom',String(v));
    ['ocaf.factor_pub','ocaf.factor_fy','ocaf.factor_pubdate','ocaf.factor_state'].concat(_wroteFc?['ocaf.factor_custom']:[]).forEach(k=>{markCycle(k);if(form[k]){form[k].origin='fr';form[k].pinned=false;}});
    renderBody();
...
async function uafApplyUas(){let n=0;
  ...
  UNITS.forEach(i=>{const r=uafRow(i);if(r.curSum>0&&r.newSum>0){const _uak='units.'+i+'.ua_custom';form=store.editForm(form,_uak,String(r.newSum));if(form[_uak]){form[_uak].origin='typed';form[_uak].pinned=true;}n++;}});   // the UAF worksheet's own total is a deliberate figure — pin it so a re-parse leaves it
...
function attnFlags(){const f=[];const u=UNITS.filter(uaUnresolved).length;if(u)f.push(u+' UA conflict'+(u>1?'s':'')+' to resolve');const sf=hasProg('rcs')?UNITS.filter(safmrUnresolved).length:0;if(sf)f.push(sf+' SAFMR conflict'+(sf>1?'s':'')+' to resolve');const A=analysis();if(hasProg('rcs')){if(A.safmrMissing)f.push(hudBlockerText()?('To pull SAFMRs, '+hudBlockerText()):'SAFMR needed for the 150% test');if(!A.safmrMissing&&A.countsMissing&&A.safmrHave)f.push('Unit counts are needed for the 150% test ('+secRef(6)+')');if(A.safmrOver)f.push(A.safmrOver+' unit type'+(A.safmrOver>1?'s':'')+' over 150% SAFMR');}rsCapacity().flags.forEach(x=>f.push(x));const ov=overrideCount();if(ov)f.push(ov+' unsaved override'+(ov>1?'s':''));if(srcOf('nonrev.enabled')==='overridden')f.push('Non-revenue units turned '+(get('nonrev.enabled')==='1'?'on':'off')+' \u2014 saves with your next \u201cUpdate property profile\u201d');if(srcOf('ns8.enabled')==='overridden')f.push('Non-Section 8 units turned '+(get('ns8.enabled')==='1'?'on':'off')+' \u2014 saves with your next \u201cUpdate property profile\u201d');if(!_rcsUpload&&(hasProg('rcs')||hasProg('ocaf')))f.push((hasProg('rcs')?'The completed RCS report isn’t uploaded':'The CA’s auto-OCAF package isn’t uploaded')+' ('+secRef(1)+')');
  ...
  return f;}
/* Three states, three marks. The rail drew two — warn, and everything else as a
   green tick — so a section nobody had touched wore the same tick as a finished
   one, and a package one click old read 9 of 10 confirmed. "Confirmed" now
...
function renderRail(_si){_si=_si||sectionIssues();
  ...
  /* A dimmed section still counts as confirmed. It IS confirmed — every
     ...
     package the ring calls ready. */
  let conf=0,cavN=0;vis.forEach(n=>{if(st[n]==='ok'||st[n]==='cav')conf++;if(st[n]==='cav')cavN++;});
  const need=vis.length-conf;
  ...
  el('rail').innerHTML='<div class="railbar nofx" id="railbar" aria-hidden="true"></div>'+vis.map(n=>`<button type="button" class="railitem" data-rsec="${n}"><span class="ri ${st[n]==='warn'?'warn':(st[n]==='empty'?'none':(st[n]==='cav'?'ok cav':'ok'))}"${st[n]==='cav'?' title="'+esc(cavWhy(_si.cav[n]))+'"':''}>${st[n]==='warn'?'!':(st[n]==='empty'?'\u25cb':'✓')}</span><span class="rname">${_secPos[n]||n}. ${SECTION_TITLES[n]}</span></button>`).join('');
  ...
  el('railprog').innerHTML=`<b>${conf} of ${vis.length} confirmed</b>${need?`<div class="warnt">${need} section${need===1?"":"s"} still to confirm</div>`:''}${cavN?`<div class="cavt">${cavN} confirmed with items left</div>`:''}<div class="track sm"><div style="width:${conf/vis.length*100}%;background:#1f5480"></div></div>`;
...
function railPick(){
  ...
  /* THE HANDOVER, and why the line alone was not enough. The line answers "the
     last heading you scrolled past", and for most of this form that named the
     PREVIOUS section: a card's final sliver tucked under the sticky chrome
     holds the line while the next card fills the whole window below it. Matt,
     2026-08-07, with three screenshots, one per boundary. Measured over the
     whole document afterwards: 27 of 150 scroll positions named a card holding
     less than a quarter of the window while another held more than half.

     So the line picks the CANDIDATE, and then the reader's eye breaks the tie:
     of the card you are leaving and the card you are entering, the one you can
     see more of wins. No tuned threshold — whichever is bigger.

     Two rules that were tried and are worse, both measured, both here so they
     are not tried again:
       · "whichever card fills most of the window", with no line at all. A
         section SHORTER than the window can be flanked by two long ones that
         each show more of themselves than it holds in total, so its row never
         lights. Six of ten rows became unreachable.
       · moving the line down to ~38% of the window. It fixes the sliver case
         but breaks the opposite one: scroll a SHORT section's heading to the
         top of the window and the line has already sunk past it into the next
         card, so the rail disagrees with the heading under your eye. Six of
         eight sections here are shorter than that offset.
     Comparing the two candidates keeps the line at 88, so a heading brought to
     the top still names its own section whatever that section's height. */
  const h=window.innerHeight||(document.documentElement||{}).clientHeight||0;
  const seen=cd=>{const r=cd.getBoundingClientRect();
    return Math.max(0,Math.min(r.bottom,h)-Math.max(r.top,RAIL_LINE));};
  let above=null,below=null;
  for(let i=0;i<cards.length;i++){const r=cards[i].getBoundingClientRect();
    if(r.top<=RAIL_LINE&&r.bottom>RAIL_LINE){
      const nx=cards[i+1];
      return +((nx&&seen(nx)>seen(cards[i]))?nx:cards[i]).getAttribute('data-sec');}
...
function syncReviewed(){const held=k=>{const c=form[k];return !!(c&&c.db_value==='1');};
  ...
  UNITS.forEach(i=>{drop('units.'+i+'.type_reviewed',typeConflict(i));drop('units.'+i+'.num_reviewed',numConflict(i));});}
function renderBody(){
  ...
  /* Re-clicking the option already selected LOCKS the cell (pins it). Leave the menu
     up so the padlock that just appeared on that line is seen, instead of closing on
     an action the reader may not have known they took. A first-time pick still closes. */
  if(_reopenBox){try{const _rb=document.querySelector('[data-box="'+_reopenBox+'"]');const _rd=_rb&&(_rb.querySelector('.uadrop:not(.cs)')||_rb.querySelector('.uadrop'));if(_rd)_rd.classList.add('open');}catch(e){}}_reopenBox=null;
...
function coupledKeys(k){   // UA, SAFMR and the effective date flattened: their *_custom cells are plain, coupled to nothing
  const OCAF_F=['ocaf.factor_custom','ocaf.factor_pub','ocaf.factor_fy','ocaf.factor_pubdate','ocaf.factor_state'];   // flattened: the value + the FR provenance that rides with it; the dead factor_src pointer is gone
...
function paintCell(k){const gb=groupOf(k);if(gb)return paintGroup(gb);
  ...
  const _sp=(typeof srcSpecAny==='function')?srcSpecAny(k):null;
  if(_sp){const fc=cellColor(_sp.srcKey);   // one colour rule, both painters — every source family (ua/safmr/date/ocaf)
...
function srcSpec(k){
  /* No resolver cells remain (all four flattened 2026-08-07): UA, SAFMR, the effective date and now the
     OCAF factor are plain *_custom cells, so nothing returns a spec here — typing stays a plain edit and
     every dropdown pick falls through to the table-driven engine (moneySrcRows / Engine B). */
  return null;
}
/* srcSpecAny: srcSpec keys on the *_custom key; the renderer/painter key a source
   cell on its *_source data-box (FR-9). Accept either — map a source key back to
   its custom partner — so cellColor(sourceKey) can reach the family logic. */
function srcSpecAny(k){let sp=srcSpec(k);if(sp)return sp;
  const cus=k.replace(/\.ua_source$/,'.ua_custom').replace(/\.safmr_source$/,'.safmr_custom').replace(/\.date_eff_source$/,'.date_eff_custom').replace(/\.factor_src$/,'.factor_custom');
  return cus!==k?srcSpec(cus):null;}
...
function wireBody(){
  ...
  document.querySelectorAll('[data-srcopt]').forEach(o=>o.addEventListener('click',e=>{e.stopPropagation();
    ...
    /* Already the selected line? Re-clicking it only LOCKS the cell — keep the menu
       open after the render so the padlock is seen (a first-time pick still closes). */
    if(o.classList.contains('sel')){const _cb=o.closest('[data-box]');_reopenBox=_cb?_cb.getAttribute('data-box'):null;}else _reopenBox=null;
    if(src.indexOf('cpick:')===0){const id=src.slice(6);
      if(key==='poc.name'){const ct=(mpdb?mpdb.listContacts():[]).find(x=>x.id===id);_pendingSnap=snapPend(POC_PICK_KEYS);if(ct)pocSelectContact(ct);_pending=POC_PICK_KEYS.slice();_refocusSel='[data-box="poc.name"] .pocname-in';}
      else{const P=DIR_PICK[key];const ct=dirList(P.kind).find(x=>x.id===id);_pendingSnap=snapPend(P.keys);if(ct)P.apply(ct);_pending=P.keys.slice();_refocusSel='[data-box="'+key+'"] .pocname-in';}
      renderBody();return;}
    if(src.indexOf('cdoc:')===0){const which=src.slice(5);
      if(key==='poc.name'){
        if(which==='ra'){const nm=raVal('poc.name');if(nm){const saved=(mpdb?mpdb.listContacts():[]).find(x=>((x.name||'').trim().toLowerCase()===nm.trim().toLowerCase()));const cc={name:nm,email:raVal('poc.email')||(saved&&saved.email)||'',phone:raVal('poc.phone')||(saved&&saved.phone)||''};_pendingSnap=snapPend(POC_PICK_KEYS);pocSelectContact(cc);if(form['poc.name']){form['poc.name'].origin='ra';form['poc.name'].pinned=true;}_pending=POC_PICK_KEYS.slice();_refocusSel='[data-box="poc.name"] .pocname-in';}}
        else{const pm=pocMatchRcs();if(pm.name){_pendingSnap=snapPend(POC_PICK_KEYS);if(pm.contact)pocSelectContact(pm.contact);else form=store.editForm(form,'poc.name',pm.name);if(form['poc.name']){form['poc.name'].origin='rcs';form['poc.name'].pinned=true;}_pending=POC_PICK_KEYS.slice();_refocusSel='[data-box="poc.name"] .pocname-in';}}
        renderBody();return;}
    ...
    if(sp){                                               // Engine A: a resolver pointer
      ...
      const _rev=sp.cusKey.replace(/_custom$/,'_reviewed');if(_rev!==sp.cusKey&&form[_rev]!==undefined)form=store.editForm(form,_rev,'1');
      _pending=[sp.srcKey];_refocusSel='[data-box="'+sp.srcKey+'"] .uatrigger';renderBody();return;}
    _pendingSnap=snapPend([key]);                         // Engine B: write the value (carried on the option) + record its origin
    const _v=o.getAttribute('data-srcv');
    if(_v!=null){form=store.editForm(form,key,String(_v));if(form[key]){form[key].origin=src;form[key].pinned=true;}}
    _pending=[key];_refocusSel='[data-box="'+key+'"] .uatrigger';renderBody();refreshIfPrereq([key]);setStatus('Pulled from '+((o.querySelector('.uasub')||{}).textContent||'the source')+'.');}));
  ...
  const lt=el('ns8Toggle');if(lt)lt.onchange=()=>{
    if(!lt.checked&&NS8.some(i=>ns8HasData(i)||numf(get('ns8.'+i+'.num_units'))>0)){
      ...
      return;}
    if(!lt.checked){NS8.forEach(i=>Object.keys(form).forEach(k=>{if(k.indexOf('ns8.'+i+'.')===0)delete form[k];}));NS8=[];}
  ...
  const rapo=el('rcsApplyOver');if(rapo)rapo.onclick=()=>{rcsFillFromParsed({force:true});setStatus('Applied the study over your entries.');};
  ...
  const rao=el('rsApplyOver');if(rao)rao.onclick=()=>{rsFillFromParsed({force:true});setStatus('Applied the schedule over your entries.');};
...
/* A statement, not a question. dialogConfirm always draws Cancel beside its
   action, which asks the reader to decide something when there is nothing to
   decide. */
function dialogAlert(title,body){
  modal('<div class="dlg-t">'+esc(title)+'</div><div class="dlg-b">'+esc(body)+'</div>'
    +'<div class="dlg-row"><span class="dlg-sp"></span><button class="btn p" id="dlgOk">OK</button></div>');
  el('dlgOk').onclick=closeModal;
}
...
/* ── Related Affordable, arriving down the tracker's pipe ─────────────────
   ...
   source-backed cell and isLocked() runs on every field; reading the tracker
   means a targetFor across the whole schedule. The key carries the tracker's
   own generation so a re-seeded CSV invalidates it. */
let _raSnap=null;
function raSnapshot(){
  ...
  try{
    ...
    if(mine.length){
      ...
      /* THE TRAP, and the reason this is not simply targetFor(). A package
         ...
         tracker's CURRENT target would padlock next year's date onto a package
         filed in 2018 the moment somebody reopened it, and the app would be
         breaking a promise its own tooltip makes. So the package's own date
         wins; the target answers only where there is no package yet. */
      const c=activeCid?((mpdb.listCycles(activePid)||[]).find(x=>x.id===activeCid)||null):null;
      if(c)s.eff=c.effective_date||null;
      else{
        const t=H.targetFor(rows,code,mpdb.today?mpdb.today():new Date().toISOString().slice(0,10));
        s.eff=(t&&t.effective)||null;
      }
    }
  }catch(e){}
  _raSnap=s;return s;
}
/* listProperties is the portfolio the New-property picker offers. `id` is the
   tracker CODE deliberately: it lands in createProperty's raMasterId and
   becomes ra_property_code, which is the only thing that binds a record to the
   schedule. A picker that returned a display id would create properties the
   tracker could never find again. */
/* Named, and called from boot rather than written there, because a headless
   suite has no boot to run: an installer that only exists inside boot() is one
   the render suites cannot reach, so the adapter would go untested exactly
   where it is cheapest to test it. Guarded, so a provider injected before boot
   — the RA integrator's, or ?ra=1's — is the one that answers. */
function installRaSource(){
  if(typeof window==='undefined')return null;
  if(!window.RASource)window.RASource=raTrackerSource();
  return window.RASource;
}
function raTrackerSource(){return{
  listProperties:()=>{
    try{
      const H=window.RCSHap,rows=hapAll();if(!H||!rows)return [];
      const scope=H.inScope(rows),seen={},out=[];
      rows.forEach(r=>{if(scope[r.code]&&!seen[r.code]){seen[r.code]=1;out.push({id:r.code,name:r.name||r.code});}});
...
function cyclesHtml(hasAction,hap){
  ...
  /* Only where the schedule has no opinion. On a property the tracker carries,
     this door opened the UNRESTRICTED dialog — any programme, any date — so it
     could start an RCS in an OCAF year, which is the app contradicting the
     tracker that owns that fact. Everything it used to be needed for is gone:
     a second rent action in one year is a second DATE and carries its own
     schedule row and its own card, and a standalone utility allowance is now
     contents of the year's package rather than a package of its own. A
     tracker-carried property always has a way in above this — the current
     package's card, or nextUpHtml's strip and its button.
     Off the schedule there is no other door, so it stays. */
  const _tracked=!!(hap&&hap.action&&hap.action.kind!=='none');
  const start=_tracked?'':('<button class="'+(hasAction?'addrow':'btn p')+'" id="bNewCycle">'
  ...
  /* Current is the package the SCHEDULE points at — not the newest one that
     happens to exist. A property that filed 2025 and has not started 2026 showed
     its 2025 package as Current, complete and full-ringed, while its own home
     card correctly said "Start 2026 OCAF, 0%": one property, two screens, two
     answers to one question. Both now read the tracker's target.

     A date holds one package (assertPackageFree), so gathering every package
     sharing the current date is no longer a thing that can return two.

     When the target has no package yet, NOTHING here is current — the NEXT
     RENEWAL strip that nextUpHtml puts above this list is what says what to
     start, and it renders exactly when the target is unstarted. Off-tracker
     properties have no target at all, so they keep the newest package. */
  const _tgt=_due?cs.find(c=>c.id===_due):null;
  const cur=_tgt?[_tgt]:(_tracked?[]:[cs.find(c=>c.dominant)||cs[0]]);
  ...
  const card=c=>'<div class="cycard'+(c.dominant?' dom':'')+'" data-cyopen="'+c.id+'">'
    ...
    +'<span class="cy-dom">Current</span>'
    +stChip(c,scOf(c))+'</div>'
...
async function openCycleForm(cid){
  ...
  fixSavedToggles();applyChecklistDefaults();deriveUnits();migrateFlatCells();
...
function renderFormHeader(){
  ...
  if(wrap&&cy){
    ...
    const ocafDis=!progEnabled('ocaf')||(!on('ocaf')&&on('rcs'))||(sched==='ocaf'&&on('ocaf'));
    wrap.innerHTML=pill('rcs',rcsDis,(sched==='rcs'&&on('rcs'))?SCHED:(rcsDis?EXCL:(on('rcs')?'Remove the RCS from this package':'Add an RCS to this package')),sched==='rcs'&&on('rcs'))
      +pill('ocaf',ocafDis,(sched==='ocaf'&&on('ocaf'))?SCHED:(!progEnabled('ocaf')?'Not available in this version':(ocafDis?EXCL:(on('ocaf')?'Remove the OCAF from this package':'Add an OCAF to this package'))),sched==='ocaf'&&on('ocaf'))
      +pill('uaf',!progEnabled('uaf'),progEnabled('uaf')?(on('uaf')?'Remove the UAF from this package':'Add a UAF to this package'):'Not available in this version')
...
async function toggleCycleProg(p){
  if(!activeCid)return;
  if(!progEnabled(p)){setStatus((PROG_NAMES[p]||p.toUpperCase())+' is not available in this version.');return;}
...
function buildRentAnalysisBytes(){
  ...
  const rows=UNITS.map(i=>({type:ut(get('units.'+i+'.br'),get('units.'+i+'.ba'),get('units.'+i+'.label')),
    ...
    ua:uaHas('units.'+i+'.ua_custom')?uaResolvedOf(i):null,safmr150:safmrResolvedOf(i)>0?safmrResolvedOf(i):null}));
...
async function genPackage(){
  ...
  /* A refusal has to land where the press did. Both of these used to answer with
     setStatus and return — and #status is in the toolbar at the top of the page
     while Generate is in the sticky footer at the bottom, some 760px away at a
     900px window. The press produced no visible response at all, so the honest
     reading of the app was that the button was broken. Same refusals, said in
     the place the reader is looking, and the one that can be acted on says
     where to go. */
  if(!(window.RCSGen&&window.PDFLib)){
    dialogAlert('Still loading','The document generator has not finished loading. Give it a moment and press Generate again.');return;}
  if(numf(get('units.'+UNITS[0]+'.num_units'))<=0){
    dialogConfirm('A unit count is needed first',
      'The rent schedule cannot be built without one. Add the number of units for the first unit type in '
      +esc(secRef(6))+', then press Generate again.','Take me there',false,()=>gotoSection(6,'units.'+UNITS[0]+'.num_units'));
    return;}
...
async function selftestSeed(){
  ...
  U.forEach((u,i)=>{
    ...
    m['units.'+i+'.ua_custom']=String(u.ua!=null&&u.ua!==''?u.ua:u.uaR);   // the one allowance value, schedule-first (RS)
    m['units.'+i+'.safmr_custom']=String(u.safmr);   // SAFMR flattened: the one 150% ceiling value
  });
  ...
  try{await mpdb._setHapRows([
    {'Property Code':'ST001','Property Name':'Selftest Gardens','Portfolio Mgr':'Jordan Doe',
...
__rcsFill:(opts)=>rcsFillFromParsed(opts),/* The same door for the rent schedule. Filling FROM a parse is the state where
  ...
  __rsFill:(opts)=>rsFillFromParsed(opts),__UNITS:()=>UNITS.slice(),__moneySrcRows:(k)=>moneySrcRows(k),__rcsChecks:()=>rcsChecks(),__rcsTag:(k)=>rcsTag(k),__rsTag:(k)=>rsTag(k),__srcTags:(k)=>srcTags(k),__rcsFillKeys:()=>rcsFillKeys(),__fillRecords:()=>({rs:_rsFill,rcs:_rcsFill}),
  ...
  __seedHap:async(rows)=>{_hapCache=null;_raSnap=null;await mpdb._setHapRows(rows);},
  __menuView:()=>menuView,
  ...
  /* Synchronous, for a sweep: railSync defers through rAF, so a loop that
     scrolls and reads the DOM in one turn reads the PREVIOUS answer at every
     step — a sweep built that way measures nothing and looks authoritative. */
  railPickNow:()=>{railApply();return _railActive;},
  railLine:()=>RAIL_LINE,
  ...
  /* Test-only hooks for the program gate (Task 6's browser visual check):
     read the resolved flag, or flip it to simulate a narrowed build. */
  __progEnabled:(p)=>progEnabled(p),__setEnabledPrograms:(a)=>{ENABLED_PROGRAMS=(a||[]).slice();},
