...
function srcSpec(k){
  /* UA, SAFMR and the effective date are no longer resolver cells (flattened 2026-08-07): their
     *_custom cells are plain, so none returns a spec here — typing stays a plain edit and their
     dropdown picks fall through to the table-driven engine (moneySrcRows). OCAF is the last resolver. */
  if(k==='ocaf.factor_custom')return{srcKey:'ocaf.factor_src',cusKey:k,names:['fr'],
    fallback:(get('ocaf.factor_pub')?'fr':'custom'),resolve:ss=>ss==='fr'?String(get('ocaf.factor_pub')||''):'',figKey:()=>'ocaf.factor_pub'};
  return null;
}
