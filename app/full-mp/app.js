...
function missingBySection(){
  ...
  try{const S=packageScore();
    const add=m=>{(m||[]).forEach(x=>{if(x&&x.sec!=null)(out[x.sec]=out[x.sec]||[]).push(x);});};
    add(S&&S.profile&&S.profile.missing);   // gate 1: is this a real property record
    add(S&&S.blockers);
    ((S&&S.docs)||[]).forEach(d=>add(d.missing));
  }catch(e){}
  return out;}
...
function sectionStatus(n){
  ...
  if((missingBySection()[n]||[]).length)return 'warn';
...
function renderRail(){const vis=visibleSections();const st={};vis.forEach(n=>st[n]=(n===7?'ok':sectionStatus(n)));let conf=0;vis.forEach(n=>{if(st[n]==='ok')conf++;});const need=vis.length-conf;
  ...
  el('rail').innerHTML='<div class="railbar nofx" id="railbar" aria-hidden="true"></div>'+vis.map(n=>`<button type="button" class="railitem" data-rsec="${n}"><span class="ri ${st[n]==='warn'?'warn':(st[n]==='empty'?'none':'ok')}">${st[n]==='warn'?'!':(st[n]==='empty'?'\u25cb':'✓')}</span><span class="rname">${_secPos[n]||n}. ${SECTION_TITLES[n]}</span></button>`).join('');
