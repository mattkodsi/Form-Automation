...
function pkgCard(){
  ...
  /* Every row's tick is computed, for every program. The OCAF/UAF card used to
     ...
     asserting six documents are in the package without asking whether one of
     them could be written, which is FORM-RULES §15 exactly. */
  const rows=S.docs.map(d=>{
    const t=d.ready?'':' class="draft-off" title="Needs '+esc(d.missing.map(x=>x.label).join(', '))+'"';
    return '<span'+t+'><i class="dtick">'+(d.ready?'✓':'')+'</i>'+esc(d.label)+'</span>';}).join('');
  const src=hasProg('rcs')
    ?(_rcsUpload?esc(_rcsUpload.name)+' — goes in as document 04':'Upload the completed RCS report in '+secRef(1)+' — it becomes document 04 of the package.')
    :(_rcsUpload?esc(_rcsUpload.name)+' uploaded — the package’s source document':esc(srcDocLabel().title)+(srcDocLabel().need?' goes in '+secRef(1):' — optional, '+secRef(1)));
  const title=hasProg('rcs')?(_rcsUpload?'RCS report uploaded':'RCS report needed')
    :(cycleProgs().map(x=>PROG_NAMES[x]||x).join(' + ')+' package');
  ...
  const gap=(list,cls,head)=>list.length?('<div class="ccsub" style="margin-top:7px;color:#33405c"><b>'+head+'</b></div><div class="gpf-wrap">'+list.map(x=>gpf(x,cls)).join('')+'</div>'):'';
  return '<div class="ccard"><div class="cck">THIS PACKAGE</div>'
    +'<div class="cctitle" style="font-size:15px">'+title+'</div><div class="ccsub">'+src+'</div>'
    +'<div class="ccsub" style="margin-top:7px;color:#33405c"><b>'+ringSvg(S.pct,30)+' '+esc(window.RCSScore.scoreCaption(S))+'</b></div>'
