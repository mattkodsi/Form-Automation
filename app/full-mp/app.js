...
function wireBody(){
  ...
  document.querySelectorAll('[data-dellihtc]').forEach(b=>b.addEventListener('click',()=>{const i=+b.getAttribute('data-dellihtc');['br','ba','avg_rent'].forEach(s=>delete form['lihtc.'+i+'.'+s]);LIHTC=LIHTC.filter(x=>x!==i);if(!LIHTC.length)form=store.editForm(form,'lihtc.enabled','');renderBody();setStatus(LIHTC.length?'':'Last LIHTC unit type removed — LIHTC section turned off.');}));
