...
function renderFieldSection(sec){const cols=[[],[]];sec.fields.forEach(f=>cols[f.col].push(fieldCell(f)));return card(sec.n,sectionPill(sec.n),`<div class="cols${sec.n===5?' nl':''}"><div>${cols[0].join('')}</div><div>${cols[1].join('')}</div></div>`);}
