...
function renderRents(){
  ...
  let lh=`<div class="pdhead lihtchead"><label class="lihtcflag"><input type="checkbox" id="lihtcToggle"${lhOn?' checked':''}><span>This property has LIHTC units</span></label>${lhOn?'<span class="sub">listed on the rent schedule as unit type + average unit rent</span>':''}</div>`;
  if(lhOn){
