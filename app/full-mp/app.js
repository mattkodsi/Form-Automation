...
let form=store.emptyForm(); let UNITS=[0]; let NONREV=[]; let NS8=[]; let PRINCIPALS=[0]; let _undoStack=[]; let _undoNR=[]; let _undoLI=[]; let _pending=null,_refocusSel=null,_pendingSnap=null; let _rcsUpload=null;
...
function visibleSections(){const pr=cycleProgs();const has=p=>pr.indexOf(p)>=0;
  if(has('rcs'))return [1,2,12,3,4,5,6,7,8,9];                 // the classic RCS flow
...
function renderFieldSection(sec){const cols=[[],[]];sec.fields.forEach(f=>cols[f.col].push(fieldCell(f)));return card(sec.n,sectionPill(sec.n),`<div class="cols${sec.n===4||sec.n===5?' nl':''}"><div>${cols[0].join('')}</div><div>${cols[1].join('')}</div></div>`);}
...
function renderPrincipals(){
  const rows=PRINCIPALS.map(i=>{const nk='principals.'+i+'.name',tk='principals.'+i+'.title';const nc=CLR[srcOf(nk)]||CLR.new,tc=CLR[srcOf(tk)]||CLR.new;
    return `<div class="prow"><div class="field"><div class="fbox" data-box="${nk}" style="background:${nc[1]};border-left-color:${nc[0]}"><input type="text" data-k="${nk}" value="${esc(get(nk))}" autocomplete="off"></div>${ovNote(nk)}</div><div class="field"><div class="fbox" data-box="${tk}" style="background:${tc[1]};border-left-color:${tc[0]}"><input type="text" data-k="${tk}" value="${esc(get(tk))}" autocomplete="off"></div>${ovNote(tk)}</div><div class="urx"><button class="trash" data-delprin="${i}" title="Delete">\u2715</button></div></div>`;}).join('');
  return card(12,sectionPill(12),`<div class="pbnote">The principals comprising the ownership entity, as they appear in Part G of the rent schedule. The signatory title in ${secRef(3)} selects its principal from this list.</div><div class="prhead"><span>Principal name</span><span>Title</span><span></span></div>${rows}<div class="addrow" id="prinAdd">+ Add principal</div>`);}
...
function wireBody(){
  ...
  document.querySelectorAll('[data-delprin]').forEach(b=>b.addEventListener('click',()=>{const i=+b.getAttribute('data-delprin');Object.keys(form).forEach(k=>{if(k.indexOf('principals.'+i+'.')===0)delete form[k];});PRINCIPALS=PRINCIPALS.filter(x=>x!==i);if(!PRINCIPALS.length)PRINCIPALS=[0];renderBody();setStatus('Principal removed.');}));
  const pa=el('prinAdd');if(pa)pa.onclick=()=>{PRINCIPALS.push((PRINCIPALS.length?Math.max.apply(null,PRINCIPALS):-1)+1);renderBody();setStatus('');};
