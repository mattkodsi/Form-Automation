...
function tintFill(k,keys,c){if(partHot(k))return cellColors(k)[1];
  ...
  return null;}
function addrParts(keys,c){return keys.map((k,i)=>{const fill=tintFill(k,keys,c);if(!fill)return null;
  return {fill,bar:partHot(k)?cellColors(k)[0]:null,
    leftJoin:i>0&&tintFill(keys[i-1],keys,c)===fill,
    rightJoin:i<keys.length-1&&tintFill(keys[i+1],keys,c)===fill};});}
