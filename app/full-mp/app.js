...
function selectCell(f){const c=cellColors(f.k);let dd=csDrop(f.k,f.opts,f.ph||'Select…');
...
/* ---- Tier 2: positioned text extraction for flattened copies ----
   Interprets content streams (q/Q + cm stack, Tm/Td/TD/T*, Tj/TJ/quote),
   yielding text runs with page coordinates; runs are then mapped onto the
   template's known field rectangles. Literal + 1-byte hex strings cover the
   DocuSign class; 2-byte CID copies need ToUnicode work (next increment). */
...
function rsRuns(txt){ // one content stream -> [{str,x,y}]
  const runs=[];let cm=[1,0,0,1,0,0],stack=[],tm=null,tlm=null,TL=0;
  const mul=(a,b)=>[a[0]*b[0]+a[1]*b[2],a[0]*b[1]+a[1]*b[3],a[2]*b[0]+a[3]*b[2],a[2]*b[1]+a[3]*b[3],a[4]*b[0]+a[5]*b[2]+b[4],a[4]*b[1]+a[5]*b[3]+b[5]];
  const emit=str=>{if(!str)return;const m=mul(tm||[1,0,0,1,0,0],cm);runs.push({str:str,x:m[4],y:m[5]});};
  ...
  const deHex=h=>{h=h.replace(/\s+/g,'');let o='';if(h.length>=4&&/^00/.test(h)){for(let i=0;i+3<h.length;i+=4)o+=String.fromCharCode(parseInt(h.substr(i,4),16));}else{for(let i=0;i+1<h.length;i+=2)o+=String.fromCharCode(parseInt(h.substr(i,2),16));}return o;};
  const re=/\((?:[^()\\]|\\.)*\)|<[0-9A-Fa-f\s]*>|\[(?:[^\[\]\\()]|\((?:[^()\\]|\\.)*\))*\]\s*TJ|[-.0-9]+|[A-Za-z'*"]+/g;
  let m,nums=[],lastStr=null;
  while((m=re.exec(txt))){const t=m[0];
    if(t[0]==='('){lastStr=deLit(t.slice(1,-1));continue;}
    if(t[0]==='<'){lastStr=deHex(t.slice(1,-1));continue;}
    if(t[0]==='['){const parts=[];const ri=/\((?:[^()\\]|\\.)*\)|<[0-9A-Fa-f\s]*>/g;let mm;while((mm=ri.exec(t))){parts.push(mm[0][0]==='('?deLit(mm[0].slice(1,-1)):deHex(mm[0].slice(1,-1)));}emit(parts.join(''));nums=[];lastStr=null;continue;}
    if(/^[-.0-9]+$/.test(t)){nums.push(parseFloat(t));continue;}
    switch(t){
      case 'q':stack.push(cm.slice());break;
      case 'Q':cm=stack.pop()||[1,0,0,1,0,0];break;
      ...
      case 'Td':if(nums.length>=2){tlm=mul([1,0,0,1,nums[nums.length-2],nums[nums.length-1]],tlm||[1,0,0,1,0,0]);tm=tlm.slice();}break;
      case 'TD':if(nums.length>=2){TL=-nums[nums.length-1];tlm=mul([1,0,0,1,nums[nums.length-2],nums[nums.length-1]],tlm||[1,0,0,1,0,0]);tm=tlm.slice();}break;
      ...
      case "'":tlm=mul([1,0,0,1,0,-TL],tlm||[1,0,0,1,0,0]);tm=tlm.slice();emit(lastStr);lastStr=null;break;
    }
    nums=[];}
  return runs;}
async function rsTextPages(bytes){ // BT-bearing ASCII streams in document order = page candidates
  const raw=new TextDecoder('latin1').decode(bytes);const re=/stream\r?\n/g;let m;const pages=[];
  while((m=re.exec(raw))){const st=m.index+m[0].length;const en=raw.indexOf('endstream',st);if(en<0)continue;
    let txt=null;try{txt=await rsInflate(bytes.slice(st,en));}catch(e){txt=raw.slice(st,en);}
    try{const probe=txt.slice(0,20000);const asc=(probe.match(/[\x20-\x7e\r\n]/g)||[]).length;
      if(probe.length&&asc/probe.length>0.85&&txt.indexOf('BT')>=0)pages.push(rsRuns(txt));}catch(e){}}
  return pages;}
...
async function rsReadTextTier(bytes){ // flattened copy -> same parsed shape as tier 1, or null
  const rects=await rsFieldRects();const pages=await rsTextPages(bytes);if(!pages.length)return null;
  // page association: the page holding "Part G" style entity text is template page 1; rows page is 0
  const joined=pages.map(rs=>rs.map(r=>r.str).join(' '));
  let pg0=-1,pg1=-1;
  ...
  if(pg0<0&&pages.length)pg0=0;if(pg1<0&&pages.length>1)pg1=pages.length-1;
  const hit=(pgIx,tplPg)=>{const out={};if(pgIx<0)return out;
    for(const id in rects){const rc=rects[id];if(rc.pg!==tplPg)continue;
      const inside=pages[pgIx].filter(r=>r.x>=rc.x-2&&r.x<=rc.x+rc.w+2&&r.y>=rc.y-3&&r.y<=rc.y+rc.h+5).sort((a,b)=>a.x-b.x);
      const v=inside.map(r=>r.str).join('').trim();if(v)out[id]=v;}
    return out;};
  const F=Object.assign(hit(pg0,0),hit(pg1,1));
  ...
  const dt=rsDateISO(V(3));if(dt)outp.scalars['rs_date']=dt;
  if(V(197))outp.scalars['owner.entity_name']=V(197);
  if(V(205))outp.scalars['owner.entity_type_other']=V(205);
  [206,208,210,212,214,216,218,220,222,224,226].forEach(id=>{const nm=V(id),tt=V(id+1);if(nm||tt)outp.principals.push({name:nm,title:tt});});
  for(let r=0;r<11;r++){const b=7+r*8;const t=V(b);const n=rsNum(V(b+1)),cr=rsNum(V(b+2)),ua=rsNum(V(b+4));
    if((t||n||cr)&&cr!==''&&cr>0)outp.units.push({type:t,count:n,rent:cr,ua:ua});}
  // quality gate: totals cross-check when present, else demand real substance
  const tot=rsNum(V(95));const sum=outp.units.reduce((a,u)=>a+(u.count&&u.rent?u.count*u.rent:0),0);
  const ok=outp.units.length>0&&(tot===''||Math.abs(sum-tot)<=Math.max(2,outp.units.length));
  return ok?outp:null;}
async function parseRsPdf(bytes){
  ...
  if(await rsDetectText(bytes)){let tp=null;try{tp=await rsReadTextTier(bytes);}catch(e){}
    if(tp)return {kind:'fields',parsed:tp};
    return {kind:'text',parsed:null};}
  return {kind:'scan',parsed:null};}
...
function overrideCount(){const grouped=new Set();for(const b in ADDR_GROUPS)ADDR_GROUPS[b].forEach(k=>grouped.add(k));UNITS.forEach(i=>{grouped.add('units.'+i+'.br');grouped.add('units.'+i+'.ba');});
  ...
  return c;}
function attnFlags(){const f=[];const u=UNITS.filter(uaUnresolved).length;if(u)f.push(u+' UA conflict'+(u>1?'s':'')+' to resolve');const sf=hasProg('rcs')?UNITS.filter(safmrUnresolved).length:0;if(sf)f.push(sf+' SAFMR conflict'+(sf>1?'s':'')+' to resolve');const tc=UNITS.filter(i=>typeUnresolved(i)||numUnresolved(i)).length;if(tc)f.push(tc+' unit type/count conflict'+(tc>1?'s':'')+' to resolve');const A=analysis();if(hasProg('rcs')){if(A.safmrMissing)f.push('SAFMR needed for the 150% test');if(A.safmrOver)f.push(A.safmrOver+' unit type'+(A.safmrOver>1?'s':'')+' over 150% SAFMR');}rsCapacity().flags.forEach(x=>f.push(x));const ov=overrideCount();if(ov)f.push(ov+' unsaved override'+(ov>1?'s':''));if(srcOf('nonrev.enabled')==='overridden')f.push('Non-revenue units turned '+(get('nonrev.enabled')==='1'?'on':'off')+' \u2014 saves with your next \u201cUpdate database\u201d');if(srcOf('ns8.enabled')==='overridden')f.push('Non-Section 8 units turned '+(get('ns8.enabled')==='1'?'on':'off')+' \u2014 saves with your next \u201cUpdate database\u201d');if(!_rcsUpload&&(hasProg('rcs')||hasProg('ocaf')))f.push((hasProg('rcs')?'The completed RCS report isn’t uploaded':'The CA’s auto-OCAF package isn’t uploaded')+' ('+secRef(1)+')');
...
function wireBody(){
  ...
  const nt=el('nonrevToggle');if(nt)nt.onchange=()=>{
    if(!nt.checked&&NONREV.some(i=>nonrevHasData(i)||numf(get('nonrev.'+i+'.num_units'))>0)){
      nt.checked=true;
      dialogConfirm('Turn off non-revenue units?','The section still holds '+NONREV.length+' row'+(NONREV.length>1?'s':'')+' with values. Turned off, the rows are left out of every generated document, and your next \u201cUpdate database\u201d removes them for good \u2014 re-check the box before saving to bring them back.','Turn off',true,()=>{form=store.editForm(form,'nonrev.enabled','');renderBody();setStatus('Non-revenue units off \u2014 the hidden rows are removed on your next Update database.');});
      return;}
