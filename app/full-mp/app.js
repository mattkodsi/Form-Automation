...
function showPackageModal(nm,docs,combined,missingRcs,missingLh,capMsgs,blocked,warnOf){
  ...
  const rows=ORDER.map(o=>{const lab=o[0],hit=byLabel[lab];
    ...
    const req=hit?[]:gapOf(lab), sug=sugOf(lab);
  ...
  document.querySelectorAll('[data-dldoc]').forEach(b=>{const go=()=>{const d=docs[+b.getAttribute('data-dldoc')];dlPdf(d.bytes,d.file+'.pdf');};
    const inPop=e=>!!(e.target&&e.target.closest&&e.target.closest('.gpw'));
    b.onclick=e=>{if(inPop(e))return;go();};
    // a div carrying role="button" owes the keyboard both of a button's keys
    b.onkeydown=e=>{if(inPop(e))return;if(e.key==='Enter'||e.key===' '){e.preventDefault();e.stopPropagation();go();}};});
  /* Hover opens it; a press pins it open, because a touch screen has no hover
     and a keyboard reaches the chip before it reaches the list. :focus-within
     covers the tab route, this covers the tap. */
  document.querySelectorAll('.gpw>.gshort').forEach(b=>b.addEventListener('click',e=>{
    e.preventDefault();e.stopPropagation();
    const w=b.parentElement;const on=w.classList.contains('open');
    document.querySelectorAll('.gpw.open').forEach(x=>x.classList.remove('open'));
