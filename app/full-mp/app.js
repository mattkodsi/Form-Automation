...
function renderRents(){
  ...
  let lh=`<div class="pdhead"><label class="lihtcflag"><input type="checkbox" id="lihtcToggle"${lhOn?' checked':''}><span>This property has LIHTC units</span></label>${lhOn?' <span class="sub">listed on the rent schedule as unit type + average unit rent</span>':''}</div>`;
  if(lhOn){
...
function wireBody(){
  ...
  document.querySelectorAll('[data-delnonrev]').forEach(b=>b.addEventListener('click',()=>{const i=+b.getAttribute('data-delnonrev');['use','br','ba','num_units','rent'].forEach(s=>delete form['nonrev.'+i+'.'+s]);NONREV=NONREV.filter(x=>x!==i);if(!NONREV.length)form=store.editForm(form,'nonrev.enabled','');renderBody();setStatus(NONREV.length?'':'Last non-revenue unit removed — Part D section turned off.');}));
  document.querySelectorAll('[data-dellihtc]').forEach(b=>b.addEventListener('click',()=>{const i=+b.getAttribute('data-dellihtc');['br','ba','num_units','avg_rent'].forEach(s=>delete form['lihtc.'+i+'.'+s]);LIHTC=LIHTC.filter(x=>x!==i);if(!LIHTC.length)form=store.editForm(form,'lihtc.enabled','');renderBody();setStatus(LIHTC.length?'':'Last LIHTC unit type removed — LIHTC section turned off.');}));
...
function zeroUnitBlock(keys){const zk=keys.find(k=>/^(units|nonrev|lihtc)\.\d+\.num_units$/.test(k));if(zk&&numf(get(zk))<=0){setStatus('Cannot save zero units — enter a unit count or delete the row.');return true;}return false;}
...
function requestSave(afterSave){
  const mu=countlessUnits().filter(unitHasData);
  ...
  if(total){const parts=[];if(mu.length)parts.push(mu.length+' revenue');if(mn.length)parts.push(mn.length+' non-revenue');if(ml.length)parts.push(ml.length+' LIHTC');
...
async function openForm(program){activeProgram=program||'RCS';await mpdb.setActive(activePid);await refreshSnap();form=await store.fillForm();fixSavedToggles();applyChecklistDefaults();deriveUnits();renderFormHeader();renderBody();show('Form');window.scrollTo(0,0);ensureHudSafmr({});}
