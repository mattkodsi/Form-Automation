...
function readSubject(txt,S,warn){
  ...
  /* Fairview writes the number inside the name: "Fairview Homes (Section 8
...
/* \s* between the type and the count, not \s+: Cornerstone's Golden Link welds
...
/* The SAME grid, welded ALL the way — no space anywhere between the columns, so
   the whole row arrives as "1/112600$525$0.88Y" (br/ba, count, sf, then $rent,
   $psf, Y). Gill's "Final RCS" template (Newberry, Fairview, Friendship) prints it
   this way, with fractional baths too (2/1.5, 4/2.5). The count and sf have no
   delimiter, but sf for an apartment is three digits or "N,NNN" and the '$' anchors
   the rent, so a lazy count followed by a 3-digit-or-comma sf resolves the split.
   Tried after ROW_GRID (which needs the spaces) and before ROW_MAIN. */
const ROW_GRID2=/^(\d)\s*\/\s*(\d(?:\.\d)?)(\d+?)(\d,\d{3}|\d{3})\$([\d,]+)\$?([\d.]+)?\s*(?:([YN])\b|$)/i; // br/ba count sf $rent $psf grid — fully welded
...
function readTables(txt,pi,units,totals,seen){
  ...
  txt.forEach(function(t){
    ...
    if((m=t.match(ROW_GRID2))){                      // FULLY welded grid (Gill "Final RCS": Newberry/Fairview/Friendship)
      const u=upsert(units,m[1]+'/'+m[2],pi,money(m[5]),money(m[4]));
      u.count=money(m[3]);u.sf=money(m[4]);u.psf=dec(m[6]);u.grid=/^y$/i.test(m[7]);
      seen.push({type:norm(u.type),rent:money(m[5])});
      if(u.proposed==='')u.proposed=money(m[5]);
