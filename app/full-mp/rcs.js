...
function upsert(units,type,page,count){
  ...
  const u={type:p.type,br:p.br,ba:p.ba,count:'',sf:'',proposed:'',ua:'',safmr:'',safmr_base:'',psf:'',grid:false,page:page,alias:null};
...
async function readLetter(rd){
  ...
  /* The ceiling must be 1.5x the base the study itself printed. */
  units.forEach(function(u){
    if(u.safmr!==''&&u.safmr_base!==''&&Math.abs(Math.round(u.safmr_base*1.5)-u.safmr)>2)
