...
async function findLetter(rd){
  ...
  /* The sign-off can sit on its OWN page after the tables, carrying no rent table of its own —
     Oak Center's Belfry letter prints "Sincerely, / <names> / <credentials>" on a page with no
...
function readSignature(txt,S){
  ...
  for(let i=si+1,seen=0;i<txt.length&&seen<7;i++){
    ...
    /* "Name, Title" on one printed line — the appraiser signs "Kyle L. Bjerke, Partner". Take the
