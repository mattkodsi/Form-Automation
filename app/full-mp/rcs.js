...
/* A study TOTAL can reach us split by an extractor-injected mid-number space ("$291,420" ->
   "$2 91,420", since rsRuns turns a wide kerning gap into a space and its anti-split guard only
   protects a digit-COMMA boundary). The total regexes below capture across single spaces
   ((?:\s+[\d,]+)*); this de-spaces the capture, but ONLY when the result is one clean US-grouped
   amount — so a total that happens to be followed by another number ("$291,420 12 units") falls
   back to the first run and can never weld two separate figures into one. (2026-08-10) */
function totMoney(cap){const flat=String(cap).replace(/\s+/g,'');
  return /^\d{1,3}(,\d{3})*$|^\d+$/.test(flat)?money(flat):money((String(cap).match(/[\d,]+/)||[''])[0]);}
...
async function findLetter(rd){
  ...
  const pages=[head];
  let brk=-1;                          // the page the table-run stopped ON — already read, a sign-off candidate
  for(let j=1;j<=LETTER_TAIL&&head+j<rd.pageCount;j++){
    const k=pageKey(await get(head+j));
    if(TOC.test(k)){brk=head+j;break;}
    if(!LETTER_TABLE.test(k)){brk=head+j;break;}
    pages.push(head+j);
    if(LETTER_END.test(k))break;      // signed off: nothing after this is letter
  }
  /* The sign-off can sit on its OWN page after the tables, carrying no rent table of its own —
     a Belfry letter prints "Sincerely, / <names> / <credentials>" on a page with no
     table, so the table-tail loop stops ON that page and never hands it to readSignature, and the
     appraiser's name goes unread (2026-08-10). That page is ALREADY read, so take it without another
     page read — but only on a real closing (sincerely / respectfully submitted), never on the bare
     "Job No." footer that rides every page of the report (which is why LETTER_END is too broad here). */
  if(brk>=0&&pages.indexOf(brk)<0){const bk=pageKey(runs[brk]||[]);
    if(/sincerely|respectfullysubmitted/.test(bk)&&!TOC.test(bk))pages.push(brk);}
...
function readSignature(txt,S){
  ...
  /* Count NON-BLANK lines, not raw ones: a signed letter leaves a gap between "Sincerely,"
     ...
     (a Belfry survey, 2026-08-10 — five blank lines parsed, six did not). */
  for(let i=si+1,seen=0;i<txt.length&&seen<7;i++){
    ...
    /* "Name, Title" on one printed line — the appraiser signs "Riley T. Nguyen, Partner". Take the
       name before the comma when it reads as a person's (2-4 capitalised tokens); the title after
       it (Partner, President …) is not part of the name. Cornerstone signs this way (2026-08-10). */
    { const cm=t.match(/^([^,]+),\s*\S/); if(cm&&NAME_LINE.test(cm[1].trim())){S['appr.name']=cm[1].trim();return;} }
...
function readTables(txt,pi,units,totals,seen){
  ...
  txt.forEach(function(t){
    ...
    let m;
    if((m=t.match(/total\s+gross\s+renewal\s+rent\s*:?\s*\$([\d,]+(?:\s+[\d,]+)*)/i))){totals.grossRenewal=totMoney(m[1]);return;}
    if((m=t.match(/total\s+(?:\d{4}\s+)?gross\s+(?:safmr|fmr)\s+rent\s*:?\s*\$([\d,]+(?:\s+[\d,]+)*)/i))){totals.grossSafmrBase=totMoney(m[1]);return;}
    if((m=t.match(/150%\s*of\s*(?:the\s*)?(?:safmr|fmr)\s+gross\s+rent[^$]*\$([\d,]+(?:\s+[\d,]+)*)/i))){totals.grossSafmr150=totMoney(m[1]);return;}
    if((m=t.match(/less\s+than\s+150%\s+of\s+the\s+safmr\s+gross\s+rent\s+potential\s+of\s+\$([\d,]+(?:\s+[\d,]+)*)/i))){if(totals.grossSafmr150==null)totals.grossSafmr150=totMoney(m[1]);return;}
    if((m=t.match(/^total\s*\$([\d,]+(?:\s+[\d,]+)*)/i))){if(totals.grossRenewal==null)totals.grossRenewal=totMoney(m[1]);return;}
    if((m=t.match(/subject.{0,3}s\s+concluded\s+gross\s+rent[^$]*\$([\d,]+(?:\s+[\d,]+)*)/i))){totals.grossRenewal=totMoney(m[1]);return;}
