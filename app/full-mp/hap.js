...
(function () {
  ...
  /* ---- the primary action ----------------------------------------------
     ...
     corpus in node, where Bastrop, Fox Hill and Luther Towers already are; and
  ...
  /* A TERMINAL `EXPIRES` IS THE CONTRACT EXPIRING, NOT THE TRACKER'S HORIZON.
     ...
     renews. Measured against the export, that is wrong as a blanket rule:
       ...
       · a mid-schedule EXPIRES — one with a startable row after it — is an
         ...
         of Shoreview 75948, Roosevelt 90020, Bastrop Oak Grove 90030, Luther
       ...
       · a schedule that ENDS on an EXPIRES is 125 properties, and its date
         ...
         either way. Burt Farms expires 2029-06-26 against a contract ending
     ...
     past the EXPIRES row: Sandwich Manor (75444), Country Club Heights (79612)
     and Fox Hill (90063) — the one the original design generalised from. That is
  ...
  function actionFor(rows, code, cycles, todayISO) {
    ...
    /* Two startable rows in one calendar year means year+program cannot identify
       a package. Luther Towers (90111) is the only one of the 249 with that
