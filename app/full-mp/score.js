...
/* ── the resolvers the requirement tables need ──────────────────────────────
   ...
   THEY ARE PINNED BY test_rcs.js, and they were not always. This comment used to
   claim test_browser.js pinned them; it never did - the resolvers appear nowhere
   in that suite - and under cover of that claim BOTH source precedences drifted.
   d714cd8 moved the allowance default to the study in app.js, db.js and gen.js;
   592101a moved the SAFMR default to the study's printed table. Neither touched
   this file, so for two commits the menu and the launcher scored a record by the
   opposite rule from the one the form applied. Nothing delivered was wrong -
   uaResolvedOf here was dead code and safmrResolvedOf is only read as > 0 - which
   is exactly why it went unnoticed, and exactly why it is worth closing now
   rather than after the first caller makes it visible.
